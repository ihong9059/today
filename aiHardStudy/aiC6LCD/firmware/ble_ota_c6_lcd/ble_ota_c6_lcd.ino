...
void ledTask(void* param) {
  while (1) {
    if (ledTaskActive) {
      setColor(0, 0, 30);   // dim blue
      delay(500);
      if (ledTaskActive) ledOff();
      delay(1500);
    } else {
      delay(500);
    }
...
void setup() {
  Serial.begin(115200);
  Serial.println("UTTEC C6 — Arduino BLE OTA (ESP32-C6-LCD-1.47)");
  ...
  xTaskCreate(buttonMonitorTask, "btn", 2048, NULL, 2, NULL);
...
// ─── Loop ───
void loop() {
  // Serial command input (same as BLE CMD)
  if (Serial.available()) {
    String cmd = Serial.readStringUntil('\n');
    cmd.trim();
    if (cmd.length() > 0) {
      Serial.printf("SERIAL CMD: %s\n", cmd.c_str());
      // Reuse BLE command handler
      if (cmd.startsWith("LED_")) ledTaskActive = false;
      if (cmd == "LED_RED_ON") setColor(255, 0, 0);
      else if (cmd == "LED_GREEN_ON") setColor(0, 255, 0);
      else if (cmd == "LED_BLUE_ON") setColor(0, 0, 255);
      else if (cmd == "LED_OFF") ledOff();
      else if (cmd.startsWith("SDLOAD:")) {
        String no = cmd.substring(7);
        no.trim();
        snprintf(sdloadPath, sizeof(sdloadPath), "/firmware/%s.bin", no.c_str());
