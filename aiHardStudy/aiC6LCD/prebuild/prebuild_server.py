...
app = FastAPI(title="UTTEC C6-LCD Prebuilt Firmware Server", version="1.0")
app.add_middleware(
    CORSMiddleware,
...
if __name__ == "__main__":
    import uvicorn
    print("=" * 50)
    print("UTTEC C6-LCD Prebuilt Firmware Server")
    print(f"Catalog: {_catalog['total'] if _catalog else 0} items")
    print("http://localhost:8098")
    print("=" * 50)
    load_catalog()
    uvicorn.run("prebuild_server:app", host="0.0.0.0", port=8098, reload=True)
