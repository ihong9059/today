#!/usr/bin/env python3
"""
UTTEC C6-LCD Pre-built Firmware Generator
...
# ─── 시스템 프롬프트 (ESP32-C6-LCD-1.47) ───
SYSTEM_PROMPT = """You are an ESP32-C6 firmware generator using Arduino framework.
...
- MicroSD: SPI (MISO=GPIO5, CS=GPIO4, shares MOSI/SCLK with LCD)
  Add #include <SD.h> if using SD. SD.begin(4) to init. Use File API.
...
WS2812 RGB LED API (already initialized globally as 'pixel'):
  Adafruit_NeoPixel pixel(1, 8, NEO_RGB + NEO_KHZ800);  // already declared
...
def clean_code(code: str) -> str:
    ...
    # #include 제거 (WiFi.h 제외 — 사용자 코드에서 WiFi 사용 가능)
    lines = []
    for l in code.split("\n"):
        if l.strip().startswith("#include") and "WiFi.h" not in l and "SD.h" not in l:
            continue
        lines.append(l)
    return "\n".join(lines)
...
def merge_code(user_code: str) -> str:
    base_ino = TEMPLATE_DIR / "ble_ota_c6_lcd.ino"
    if not base_ino.exists():
        base_ino = BUILD_DIR / "ble_ota_c6_lcd.ino"
...
def main():
    parser = argparse.ArgumentParser(description="UTTEC C6-LCD Pre-built Firmware Generator")
    ...
    logging.info("=" * 60)
    logging.info("UTTEC C6-LCD Pre-built Firmware Generator")
    logging.info("=" * 60)
    ...
    if args.only:
        targets = [i for i in items if i["no"] == args.only.upper()]
    elif args.category:
        targets = [i for i in items if i["category"] == args.category.upper()]
    ...
    for idx, item in enumerate(targets, 1):
        ...
        avg = elapsed / idx
        eta = avg * (total - idx)
        logging.info(f"  진행: {idx}/{total} (성공:{success_count} 실패:{fail_count}) ETA: {eta/60:.1f}분")
