...
const parentCourses = [
  ...
  },
  {
    ...
    title: 'AI 도구 활용법',
    icon: '💬',
    description: 'Claude와 Gemini 활용 - 효과적인 AI 대화와 업무 생산성 향상',
    color: 'from-purple-500 to-pink-500',
    duration: '각 5강',
    students: 203,
    rating: 4.9,
    topics: ['프롬프트 작성', '업무 활용', '창의적 활용'],
    type: 'tool-select', // Claude / Gemini 선택
    tools: [
      { id: 'claude', name: 'Claude', icon: '🟣', description: 'Anthropic의 AI 어시스턴트' },
      { id: 'gemini', name: 'Gemini', icon: '🔵', description: 'Google의 AI 어시스턴트' },
    ],
  },
  {
...
export default function ParentCoursePage() {
  ...
  const handleStartCourse = (course: typeof parentCourses[0], toolId?: string) => {
    if (course.type === 'tool-select' && toolId) {
      router.push(`/course/parent/${course.id}/${toolId}`);
    } else {
      router.push(`/course/parent/${course.id}`);
    }
  };
          ...
          {parentCourses.map((course) => (
            <div key={course.id} className="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-lg transition">
              <div className="md:flex">
                ...
                {/* 오른쪽: 시작 버튼 */}
                <div className="p-6 bg-gray-50 md:w-72 flex flex-col justify-center">
                  {course.type === 'single' && (
                    <button
                  ...
                  {course.type === 'tool-select' && course.tools && (
                    <div className="space-y-3">
                      <p className="text-sm font-medium text-gray-700 text-center mb-2">AI 도구 선택</p>
                      {course.tools.map((tool) => (
                        <button
                          key={tool.id}
                          onClick={() => handleStartCourse(course, tool.id)}
