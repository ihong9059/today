...
# 코스 데이터 문자열 생성
def generate_course_data_str():
    result = []
    for course_id, data in courses_data.items():
        lessons_str = ',\n      '.join([
            f"{{ day: {l['day']}, title: '{l['title']}', duration: '{l['duration']}', description: '{l['description']}' }}"
            for l in data['lessons']
        ])

        course_str = f'''  '{course_id}': {{
    id: '{course_id}',
    title: '{data['title']}',
    icon: '{data['icon']}',
    color: '{data['color']}',
...
# 파일 생성
course_data_str = generate_course_data_str()
page_content = page_template.format(course_data_str=course_data_str)

# 디렉토리 생성
os.makedirs('C:/todo/today/aws/career-change/[course]', exist_ok=True)

# 파일 저장
with open('C:/todo/today/aws/career-change/[course]/page.tsx', 'w', encoding='utf-8') as f:
    f.write(page_content)

print("Course detail page created successfully!")
