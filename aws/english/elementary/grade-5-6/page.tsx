...
const days = [
  { day: 1, title: 'be동사 마스터', desc: 'am, is, are 완벽 이해' },
  { day: 2, title: '일반동사', desc: '동작을 나타내는 동사들' },
  { day: 3, title: '현재시제', desc: '지금 일어나는 일 표현하기' },
  { day: 4, title: '과거시제 입문', desc: '과거에 일어난 일 표현하기' },
  { day: 5, title: '의문문과 부정문', desc: '질문하고 아니라고 말하기' },
];

export default function Grade56Page() {
  return (
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-gray-900 font-medium">5~6학년</span>
          </div>
        </div>
      </div>
      ...
      <section className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">초등 5~6학년 영어</h1>
              <p className="text-blue-100 mt-1">be동사, 일반동사, 시제 입문</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="space-y-4">
          {days.map((item) => (
            <Link
              key={item.day}
              href={`/course/english/elementary/grade-5-6/lesson/${item.day}`}
              className="block bg-white rounded-xl shadow-md hover:shadow-lg transition p-6 border-l-4 border-blue-500"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-700 font-bold">Day {item.day}</span>
                  </div>
                  <div>
