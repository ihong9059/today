...
const days = [
  { day: 1, title: '수능 수학 전략', desc: '시간 배분, 유형별 접근' },
  { day: 2, title: '고난도 함수', desc: '함수 그래프 문제' },
  { day: 3, title: '고난도 미적분', desc: '미적분 킬러 문제' },
  { day: 4, title: '고난도 기하', desc: '벡터, 공간도형' },
  { day: 5, title: '실전 모의고사', desc: '시간 내 풀이 연습' },
];

export default function SuneungMathPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-rose-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span className="text-gray-900 font-medium">수능 수학</span>
          </div>
        </div>
      </div>

      <section className="bg-gradient-to-r from-rose-500 to-red-500 text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-4 mb-4">
            <span className="text-5xl">🎯</span>
            <div>
              <h1 className="text-3xl font-bold">수능 수학</h1>
              <p className="text-rose-100 mt-1">수능 유형별 문제 풀이</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="space-y-4">
          {days.map((item) => (
            <Link key={item.day} href={`/course/math/high/suneung/lesson/${item.day}`} className="block bg-white rounded-xl shadow-md hover:shadow-lg transition p-6 border-l-4 border-rose-500">
