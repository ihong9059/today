"""
ChatGPT (OpenAI) 챗봇 웹 서버
OpenAI API를 사용한 웹 기반 챗봇
"""
...
def status():
    """API 상태 확인"""
    if client:
        return jsonify({'status': 'ok', 'message': 'OpenAI API 연결됨'})
    else:
        return jsonify({'status': 'error', 'message': 'API 키가 설정되지 않았습니다'})

if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("  ChatGPT (OpenAI) 웹 챗봇 서버")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  http://localhost:5002 에서 실행 중...")
    print("=" * 60 + "\n")
