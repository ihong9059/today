...
export default function WeldingMetallurgyStudyPage() {
  ...
  const markComplete = () => {
    ...
    localStorage.setItem('welding-metallurgy-completed', JSON.stringify([...newCompleted]));
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-gray-800">용접야금 학습하기</h1>
          </div>
          ...
          <div className="lg:col-span-3">
                ...
                {activeTopic.questions.map((q) => {
                  ...
                  return (
                    <button key={q.id} onClick={() => { setActiveQuestionId(q.id); setShowAnswer(false); }}
                      className={`w-10 h-10 rounded-lg font-medium transition-all ${activeQuestionId === q.id ? 'bg-red-500 text-white' : isCompleted ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                      {isCompleted ? '✓' : q.id}
                    </button>
                  );
                })}
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-4">
              ...
              <div className="border-t pt-4">
                {!showAnswer ? (
                  <button onClick={() => setShowAnswer(true)} className="w-full py-3 bg-red-500 text-white rounded-lg font-medium hover:bg-red-600">정답 보기</button>
                ) : (
