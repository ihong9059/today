    ...
    questions: [
      {
        id: 1,
        question: '법인세 신고기한은?',
        answer: '사업연도 종료일부터 3개월 이내 (12월 법인: 3월 31일)',
        prompt: `TAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      },
      {
        id: 2,
        question: '법인세 중간예납이란?',
        answer: '직전 사업연도 세액의 1/2을 사업연도 개시 후 6개월 내 납부',
        prompt: `TAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      ...
      {
        id: 3,
        question: '소득세 확정신고 기한은?',
        answer: '다음연도 5월 31일 (성실신고확인대상자: 6월 30일)',
        prompt: `TAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      },
      {
        id: 5,
        question: '세금계산서합계표란?',
        answer: '매출/매입 세금계산서 내역을 거래처별로 집계한 표',
        prompt: `TAT 1급 시험 문제입니다.
...
export default function Tat1TaxStudyPage() {
  const [completedQuestions, setCompletedQuestions] = useState<Record<string, boolean>>({});
  const [expandedTopics, setExpandedTopics] = useState<Record<string, boolean>>({});

  useEffect(() => {
    const saved = localStorage.getItem('tat-1-tat1-tax-progress');
  ...
  const toggleComplete = (topicId: string, questionId: number) => {
    ...
    localStorage.setItem('tat-1-tat1-tax-progress', JSON.stringify(updated));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <Link href="/category/accounting/tat-1" className="text-gray-600 hover:text-emerald-600">TAT 1급</Link>
            <span className="text-gray-300">›</span>
            <span className="text-emerald-600 font-medium">종합세무실무</span>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">종합세무실무</h1>
