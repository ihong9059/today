    ...
    questions: [
      {
        id: 1,
        question: 'ERP란?',
        answer: '전사적자원관리. 기업의 모든 업무 프로세스를 통합 관리하는 시스템',
        prompt: `FAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      },
      {
        id: 2,
        question: '전자세금계산서란?',
        answer: '국세청 시스템을 통해 전자적으로 발급/전송하는 세금계산서',
        prompt: `FAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      ...
      {
        id: 3,
        question: '매입매출전표 입력시 필수항목은?',
        answer: '거래일자, 거래처, 계정과목, 공급가액, 부가세',
        prompt: `FAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      ...
      {
        id: 4,
        question: '부가세 예정신고 기한은?',
        answer: '각 예정신고기간 종료일의 다음달 25일',
        prompt: `FAT 1급 시험 문제입니다.
...
상세히 설명해주세요.`
      ...
      {
        id: 5,
        question: '부가세 확정신고 기한은?',
        answer: '각 확정신고기간 종료일의 다음달 25일 (1기: 7/25, 2기: 1/25)',
        prompt: `FAT 1급 시험 문제입니다.
...
export default function Fat1PracticeStudyPage() {
  const [completedQuestions, setCompletedQuestions] = useState<Record<string, boolean>>({});
  const [expandedTopics, setExpandedTopics] = useState<Record<string, boolean>>({});

  useEffect(() => {
    const saved = localStorage.getItem('fat-1-fat1-practice-progress');
  ...
  const toggleComplete = (topicId: string, questionId: number) => {
    ...
    localStorage.setItem('fat-1-fat1-practice-progress', JSON.stringify(updated));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <Link href="/category/accounting/fat-1" className="text-gray-600 hover:text-emerald-600">FAT 1급</Link>
            <span className="text-gray-300">›</span>
            <span className="text-emerald-600 font-medium">회계정보처리</span>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">회계정보처리</h1>
