...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '제품디자인 3요소는?',
        answer: '기능, 심미성, 경제성. 세 요소의 균형이 좋은 디자인',
        prompt: '제품디자인산업기사 시험 관련 질문입니다: 제품디자인 3요소는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '면(Plane)의 특성은?',
        answer: '2차원 요소, 공간 분할, 경계 형성. 평면/곡면으로 형태 표현',
        prompt: '제품디자인산업기사 시험 관련 질문입니다: 면(Plane)의 특성은? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '균형의 종류는?',
        answer: '대칭균형, 비대칭균형, 방사균형. 안정감과 역동성 표현',
        prompt: '제품디자인산업기사 시험 관련 질문입니다: 균형의 종류는? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '비례의 법칙은?',
        answer: '황금비, 등비급수, 모듈러. 아름다운 형태 비율 결정',
        prompt: '제품디자인산업기사 시험 관련 질문입니다: 비례의 법칙은? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '조형미 결정 요소는?',
        answer: '형태, 색채, 재질, 텍스처. 시각/촉각적 감성에 영향',
        prompt: '제품디자인산업기사 시험 관련 질문입니다: 조형미 결정 요소는? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('product-design-technician-form-basics-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('product-design-technician-form-basics-progress', JSON.stringify(updated));
  };
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <Link href="/category/design/product-design-technician" className="text-gray-600 hover:text-pink-600">제품디자인산업기사</Link>
            <span className="text-gray-300">›</span>
