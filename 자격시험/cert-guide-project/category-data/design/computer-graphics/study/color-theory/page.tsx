...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '색의 3속성은?',
        answer: '색상(Hue), 명도(Value), 채도(Chroma). 모든 색을 표현하는 기본 요소',
        prompt: '컴퓨터그래픽스운용기능사 시험 관련 질문입니다: 색의 3속성은? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: 'RGB와 CMYK 차이는?',
        answer: 'RGB(가산혼합, 빛, 모니터), CMYK(감산혼합, 잉크, 인쇄). 색역 차이',
        prompt: '컴퓨터그래픽스운용기능사 시험 관련 질문입니다: RGB와 CMYK 차이는? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '먼셀 색체계 표기법은?',
        answer: 'H V/C (색상 명도/채도). 10 색상환, 무채색~유채색 체계적 분류',
        prompt: '컴퓨터그래픽스운용기능사 시험 관련 질문입니다: 먼셀 색체계 표기법은? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '보색대비란?',
        answer: '색상환에서 반대편 색의 대비. 서로 강조, 채도가 높아 보이는 효과',
        prompt: '컴퓨터그래픽스운용기능사 시험 관련 질문입니다: 보색대비란? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '색온도(Color Temperature)란?',
        answer: '빛의 색상을 켈빈(K) 단위로 표현. 낮을수록 따뜻한 색, 높을수록 차가운 색',
        prompt: '컴퓨터그래픽스운용기능사 시험 관련 질문입니다: 색온도(Color Temperature)란? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('computer-graphics-color-theory-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('computer-graphics-color-theory-progress', JSON.stringify(updated));
  };
