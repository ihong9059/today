...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: 'CSS 선택자 우선순위는?',
        answer: '!important > 인라인 > ID > 클래스 > 태그. 명시도(Specificity) 계산',
        prompt: '웹디자인기능사 시험 관련 질문입니다: CSS 선택자 우선순위는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '박스 모델 구성요소는?',
        answer: 'content, padding, border, margin. box-sizing으로 계산 방식 변경',
        prompt: '웹디자인기능사 시험 관련 질문입니다: 박스 모델 구성요소는? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: 'position 속성 종류는?',
        answer: 'static(기본), relative, absolute, fixed, sticky. 배치 기준 다름',
        prompt: '웹디자인기능사 시험 관련 질문입니다: position 속성 종류는? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: 'Flexbox 주요 속성은?',
        answer: 'display:flex, justify-content, align-items, flex-direction. 1차원 레이아웃',
        prompt: '웹디자인기능사 시험 관련 질문입니다: Flexbox 주요 속성은? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '미디어 쿼리란?',
        answer: '@media로 화면 크기별 다른 CSS 적용. 반응형 웹 핵심 기술',
        prompt: '웹디자인기능사 시험 관련 질문입니다: 미디어 쿼리란? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('web-design-css-basics-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('web-design-css-basics-progress', JSON.stringify(updated));
  };
