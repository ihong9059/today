...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '선택 도구 종류는?',
        answer: '사각형, 원형, 올가미, 다각형 올가미, 자동 선택, 빠른 선택, 마술봉',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 선택 도구 종류는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '레이어 스타일 종류는?',
        answer: '그림자, 광선(Glow), 베벨, 오버레이, 획. 레이어에 효과 추가',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 레이어 스타일 종류는? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '자유 변형 단축키는?',
        answer: 'Ctrl+T. Shift(비율유지), Alt(중심기준), Ctrl(왜곡) 조합',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 자유 변형 단축키는? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '브러시 도구 옵션은?',
        answer: '크기, 경도, 불투명도, 흐름. 다양한 브러시 프리셋 활용',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 브러시 도구 옵션은? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '복제 도장(Clone Stamp) 용도는?',
        answer: '이미지 특정 영역 복제하여 그리기. 불필요 요소 제거, 패턴 연장',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 복제 도장(Clone Stamp) 용도는? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('gtq-ps-tools-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('gtq-ps-tools-progress', JSON.stringify(updated));
  };
