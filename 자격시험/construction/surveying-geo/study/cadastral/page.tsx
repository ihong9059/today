...
const topics = [
  {
    ...
    questions: [
      {
        id: 1,
        question: '지적측량의 종류를 설명하시오.',
        answer: '신규등록, 등록전환, 분할, 경계복원',
        prompt: `측량및지형공간정보기사 지적학 문제입니다.\n\n문제: 지적측량의 종류를 설명하시오.\n\n다음 순서로 설명해주세요:\n1. 핵심 개념 정리\n2. 상세 설명\n3. 적용 예시\n4. 관련 공식\n5. 연습문제 3개`
      },      {
        id: 2,
        question: '면적측정 방법을 설명하시오.',
        answer: '좌표법, 플라니미터법, 삼사법',
        prompt: `측량및지형공간정보기사 지적학 문제입니다.\n\n문제: 면적측정 방법을 설명하시오.\n\n다음 순서로 설명해주세요:\n1. 핵심 개념 정리\n2. 상세 설명\n3. 적용 예시\n4. 관련 공식\n5. 연습문제 3개`
      }
    ]
  }
...
export default function CadastralStudyPage() {
  const [completedQuestions, setCompletedQuestions] = useState<Record<string, boolean>>({});
  const [expandedTopics, setExpandedTopics] = useState<Record<string, boolean>>({});

  useEffect(() => {
    const saved = localStorage.getItem('cadastral-progress');
  ...
  const toggleComplete = (topicId: string, questionId: number) => {
    ...
    localStorage.setItem('cadastral-progress', JSON.stringify(newCompleted));
  };
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <span className="text-teal-600 font-medium">지적학</span>
          </nav>
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">지적학 학습</h1>
