...
export default function FundAdvisorPage() {
  ...
  const subjects = [
    { id: 'fund-basics', name: '펀드일반', questions: 30, icon: '📊', description: '펀드의 개념, 종류, 운용 및 투자전략' },
    { id: 'investment-recommendation', name: '투자권유', questions: 30, icon: '🤝', description: '투자권유 절차, 적합성 원칙, 설명의무' },
    { id: 'fund-law-ethics', name: '직무윤리 및 법규', questions: 20, icon: '⚖️', description: '자본시장법, 직무윤리, 투자자보호' },
  ];
  ...
  const studyPath = [
    { step: 1, title: '펀드 기초', desc: '펀드 개념과 종류 이해', duration: '2주' },
    { step: 2, title: '투자권유 실무', desc: '고객 상담 및 권유 절차', duration: '2주' },
    { step: 3, title: '법규 및 윤리', desc: '자본시장법, 직무윤리', duration: '1주' },
    { step: 4, title: '문제풀이', desc: '기출문제 및 모의고사', duration: '1주' },
  ];
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm flex-wrap">
            ...
            <span className="text-gray-300">/</span>
            <span className="text-teal-600 font-medium">펀드투자권유자문인력</span>
          </nav>
        </div>
      </header>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-teal-100">{examInfo.englishName}</p>
                <p className="text-teal-200 text-sm mt-1">주관: {examInfo.organization}</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-3">
              <div className="bg-white/20 rounded-lg px-4 py-2 text-center">
                <p className="text-2xl font-bold">{examInfo.questionCount}</p>
                <p className="text-xs text-teal-100">문항 수</p>
              </div>
              <div className="bg-white/20 rounded-lg px-4 py-2 text-center">
                <p className="text-2xl font-bold">{examInfo.examTime}</p>
                <p className="text-xs text-teal-100">시험시간</p>
              </div>
              <div className="bg-white/20 rounded-lg px-4 py-2 text-center">
                <p className="text-2xl font-bold">{examInfo.passingRate}</p>
                <p className="text-xs text-teal-100">합격률</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-8">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl shadow-sm p-6 border">
                ...
                <p className="text-gray-600 leading-relaxed">
                  펀드투자권유자문인력은 금융회사에서 펀드 상품을 판매하고 투자권유를 할 수 있는
                  전문인력 자격입니다. 은행, 증권사, 보험사 등 금융기관 취업 시 필수 자격증으로,
                  펀드 관련 업무를 수행하기 위해 반드시 취득해야 합니다.
                </p>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border">
                ...
                <div className="space-y-4">
                  <div className="bg-teal-50 rounded-lg p-4">
                    <p className="text-2xl font-bold text-teal-600">{examInfo.passingScore}</p>
                    <p className="text-sm text-teal-700">총점 100점 중 70점 이상</p>
                  </div>
                ...
                {studyPath.map((item, idx) => (
                  <div key={idx} className="relative">
                    <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-xl p-4 border border-teal-100">
                      <div className="w-8 h-8 bg-teal-500 text-white rounded-full flex items-center justify-center font-bold mb-3">
                        {item.step}
                      </div>
                      <h3 className="font-bold text-gray-800">{item.title}</h3>
                      <p className="text-sm text-gray-600 mt-1">{item.desc}</p>
                      <p className="text-xs text-teal-600 mt-2">권장: {item.duration}</p>
                    </div>
                    {idx < studyPath.length - 1 && (
                      <div className="hidden md:block absolute top-1/2 -right-2 text-teal-300">→</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="bg-gradient-to-r from-teal-500 to-cyan-500 rounded-xl p-6 text-white">
              ...
              <ul className="space-y-2 text-teal-50">
                <li>• 펀드일반 파트의 비중이 가장 높으므로 기본 개념을 확실히 익히세요</li>
                <li>• 투자권유 절차와 적합성 원칙은 실무와 직결되는 핵심 내용입니다</li>
                <li>• 법규 파트는 자본시장법 핵심 조문 위주로 학습하세요</li>
                <li>• 기출문제 반복 풀이로 출제 패턴을 파악하는 것이 중요합니다</li>
              </ul>
            </div>
          </div>
        )}
                  ...
                  <div className="bg-gradient-to-r from-teal-500 to-cyan-500 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-teal-100 text-sm">{subject.questions}문항</p>
                      </div>
                    </div>
                  </div>
                  <div className="p-4">
                    ...
                    <Link
                      href={`/category/finance/fund-advisor/study/${subject.id}`}
                      className="block w-full text-center py-2 bg-teal-50 text-teal-600 rounded-lg hover:bg-teal-100 transition font-medium"
                    >
                      학습하기 →
                    </Link>
                  </div>
        ...
        {activeTab === 'schedule' && (
          <div className="space-y-6">
                  ...
                  <tbody>
                    {schedules2026.map((schedule, idx) => (
                      <tr key={idx} className="border-b last:border-0 hover:bg-gray-50">
                        ...
                        <td className="text-center py-3 px-4 text-teal-600 font-medium">{schedule.exam}</td>
            ...
            <div className="bg-teal-50 rounded-xl p-6 border border-teal-100">
              <h3 className="font-bold text-teal-800 mb-2">📌 시험 접수 안내</h3>
        ...
        {activeTab === 'study' && (
          <div className="space-y-6">
              ...
              {subjects.map((subject) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-teal-500 font-medium">학습하기 →</span>
                  </div>
                </Link>
              ))}
            </div>
            ...
            <div className="text-center">
              <Link
                href="/category/finance/fund-advisor/exam"
