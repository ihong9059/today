...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '디자인 기본 요소는?',
        answer: '점, 선, 면, 색채, 질감, 공간. 모든 디자인의 구성 단위',
        prompt: '시각디자인산업기사 시험 관련 질문입니다: 디자인 기본 요소는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '리듬(Rhythm)이란?',
        answer: '요소의 규칙적 반복으로 시각적 운동감 표현. 일정/점진 리듬',
        prompt: '시각디자인산업기사 시험 관련 질문입니다: 리듬(Rhythm)이란? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '강조(Emphasis) 방법은?',
        answer: '크기, 색상, 형태, 위치 대비. 시선 유도, 메시지 전달',
        prompt: '시각디자인산업기사 시험 관련 질문입니다: 강조(Emphasis) 방법은? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '점진(Gradation)이란?',
        answer: '점차적 변화로 운동감과 깊이감 표현. 크기, 색상, 형태',
        prompt: '시각디자인산업기사 시험 관련 질문입니다: 점진(Gradation)이란? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '조화(Harmony)란?',
        answer: '디자인 요소들이 서로 잘 어울리는 상태. 유사조화, 대비조화',
        prompt: '시각디자인산업기사 시험 관련 질문입니다: 조화(Harmony)란? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('visual-design-technician-design-elements-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('visual-design-technician-design-elements-progress', JSON.stringify(updated));
  };
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <Link href="/category/design/visual-design-technician" className="text-gray-600 hover:text-pink-600">시각디자인산업기사</Link>
            <span className="text-gray-300">›</span>
