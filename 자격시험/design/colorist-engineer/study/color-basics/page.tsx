...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '빛의 삼원색은?',
        answer: '빨강(R), 초록(G), 파랑(B). 가산혼합, 모두 합치면 흰색',
        prompt: '컬러리스트기사 시험 관련 질문입니다: 빛의 삼원색은? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '색료의 삼원색은?',
        answer: '시안(C), 마젠타(M), 옐로(Y). 감산혼합, 모두 합치면 검정',
        prompt: '컬러리스트기사 시험 관련 질문입니다: 색료의 삼원색은? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '색온도란?',
        answer: '빛의 색상을 켈빈(K)으로 표현. 낮을수록 따뜻한 색(촛불), 높을수록 차가운 색(청천)',
        prompt: '컬러리스트기사 시험 관련 질문입니다: 색온도란? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '연색성(CRI)이란?',
        answer: '광원이 물체 색을 자연스럽게 재현하는 정도. 100에 가까울수록 좋음',
        prompt: '컬러리스트기사 시험 관련 질문입니다: 연색성(CRI)이란? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '메타머리즘이란?',
        answer: '조건등색. 조명에 따라 같아 보이던 색이 달라 보이는 현상',
        prompt: '컬러리스트기사 시험 관련 질문입니다: 메타머리즘이란? 자세히 설명해주세요.'
      },
  ];
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('colorist-engineer-color-basics-progress', JSON.stringify(updated));
  };
