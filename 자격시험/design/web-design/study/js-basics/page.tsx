...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: 'var, let, const 차이는?',
        answer: 'var(함수 스코프), let(블록, 재할당O), const(블록, 재할당X). let/const 권장',
        prompt: '웹디자인기능사 시험 관련 질문입니다: var, let, const 차이는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: 'DOM이란?',
        answer: 'Document Object Model. HTML을 객체로 표현, JS로 조작 가능',
        prompt: '웹디자인기능사 시험 관련 질문입니다: DOM이란? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '이벤트 리스너 추가법은?',
        answer: 'element.addEventListener(\'event\', function). 이벤트 버블링/캡처링',
        prompt: '웹디자인기능사 시험 관련 질문입니다: 이벤트 리스너 추가법은? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '함수 선언식과 표현식 차이는?',
        answer: '선언식(호이스팅), 표현식(변수에 할당). 화살표 함수는 this 바인딩 다름',
        prompt: '웹디자인기능사 시험 관련 질문입니다: 함수 선언식과 표현식 차이는? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '배열 메서드 map, filter 용도는?',
        answer: 'map(변환), filter(조건필터), reduce(누적). 원본 배열 불변',
        prompt: '웹디자인기능사 시험 관련 질문입니다: 배열 메서드 map, filter 용도는? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('web-design-js-basics-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('web-design-js-basics-progress', JSON.stringify(updated));
  };
