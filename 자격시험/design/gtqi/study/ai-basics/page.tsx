...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '앵커 포인트 종류는?',
        answer: '코너 포인트(꺾임), 스무스 포인트(곡선). Alt로 전환',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 앵커 포인트 종류는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '도형 도구 종류는?',
        answer: '사각형, 원, 다각형, 별, 선. Shift=정비율, Alt=중심기준',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 도형 도구 종류는? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '정렬 패널 기능은?',
        answer: '오브젝트 정렬, 분포, 기준점 설정. 아트보드/선택 기준 선택',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 정렬 패널 기능은? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '아트보드란?',
        answer: '작업 영역 캔버스. 다중 아트보드로 여러 페이지 작업',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 아트보드란? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '가이드 사용법은?',
        answer: '눈금자에서 드래그, Ctrl+; 로 토글. 정확한 배치 보조',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 가이드 사용법은? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('gtqi-ai-basics-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('gtqi-ai-basics-progress', JSON.stringify(updated));
  };
