...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: 'GTQi 시험 시간과 문제 수는?',
        answer: '90분, 4문제 (1급). 펜 도구 숙련도 중요',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: GTQi 시험 시간과 문제 수는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '주요 출제 유형은?',
        answer: '로고, 캐릭터, 아이콘, 패턴, 일러스트. 벡터 드로잉 중심',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 주요 출제 유형은? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '펜 도구 문제 대응법은?',
        answer: '앵커 포인트 최소화, 부드러운 곡선 제어, 연습 필수',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 펜 도구 문제 대응법은? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '효과 적용 문제는?',
        answer: '3D, 왜곡, 스타일화 효과. 요구사항 정확히 적용',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 효과 적용 문제는? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '파일 저장 형식은?',
        answer: 'AI 원본 필수, PDF/EPS 출력용. 저장 전 확인',
        prompt: 'GTQi(일러스트) 시험 관련 질문입니다: 파일 저장 형식은? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('gtqi-gtqi-practical-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('gtqi-gtqi-practical-progress', JSON.stringify(updated));
  };
