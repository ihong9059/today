...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: '누끼 따기 방법은?',
        answer: '펜 도구, 빠른 선택, 채널 활용, Select and Mask. 배경 제거',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 누끼 따기 방법은? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '자연스러운 합성 방법은?',
        answer: '색감 통일, 광원 일치, 그림자 추가, 경계 페더링',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 자연스러운 합성 방법은? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '레이어 마스크 vs 클리핑 마스크는?',
        answer: '레이어 마스크(개별), 클리핑 마스크(하위 레이어 기준 자르기)',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 레이어 마스크 vs 클리핑 마스크는? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '그룹(Group) 활용은?',
        answer: '레이어 정리, 일괄 효과 적용, 마스크 적용. 작업 효율화',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 그룹(Group) 활용은? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '스마트 필터 장점은?',
        answer: '비파괴 필터 적용, 재조정 가능, 마스크로 부분 적용',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 스마트 필터 장점은? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('gtq-ps-composite-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('gtq-ps-composite-progress', JSON.stringify(updated));
  };
