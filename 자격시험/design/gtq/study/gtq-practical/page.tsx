...
export default function StudyPage() {
  ...
  const questions = [
      {
        id: 1,
        question: 'GTQ 시험 시간과 문제 수는?',
        answer: '90분, 4문제 (1급). 2급은 60분. 시간 배분 중요',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: GTQ 시험 시간과 문제 수는? 자세히 설명해주세요.'
      },
      {
        id: 2,
        question: '주요 출제 유형은?',
        answer: '사진 보정, 합성, 포스터/배너, 필터 효과. 다양한 기능 활용',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 주요 출제 유형은? 자세히 설명해주세요.'
      },
      {
        id: 3,
        question: '자주 사용되는 필터는?',
        answer: 'Blur, Sharpen, Distort, Stylize 계열. 문제 요구사항 확인',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 자주 사용되는 필터는? 자세히 설명해주세요.'
      },
      {
        id: 4,
        question: '텍스트 효과 적용법은?',
        answer: '레이어 스타일, 와프, 마스크, 필터. 다양한 타이포 효과',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 텍스트 효과 적용법은? 자세히 설명해주세요.'
      },
      {
        id: 5,
        question: '시간 관리 전략은?',
        answer: '쉬운 문제 먼저, 유형별 시간 배분, 중간 저장 필수',
        prompt: 'GTQ(그래픽기술자격) 시험 관련 질문입니다: 시간 관리 전략은? 자세히 설명해주세요.'
      },
  ];

  useEffect(() => {
    const saved = localStorage.getItem('gtq-gtq-practical-progress');
    if (saved) {
  ...
  const toggleComplete = (id: number) => {
    ...
    localStorage.setItem('gtq-gtq-practical-progress', JSON.stringify(updated));
  };
