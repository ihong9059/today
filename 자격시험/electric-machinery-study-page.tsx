          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  key={ai.id}
                  onClick={() => setSelectedAI(ai.id as typeof selectedAI)}
                  className={`px-3 py-1 rounded-full text-sm font-medium transition ${selectedAI === ai.id ? 'text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                  style={selectedAI === ai.id ? { backgroundColor: ai.id === 'claude' ? '#f97316' : ai.id === 'chatgpt' ? '#10b981' : '#3b82f6' } : {}}
                >
                  {ai.name}
                </button>
              ))}
                ...
                {isExpanded && (
                  <div className="border-t bg-gray-50 p-4">
                    ...
                    <div className="flex gap-3">
                      <a
                        href={getAIUrl(q.prompt)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 py-3 rounded-lg text-white font-medium text-center transition hover:opacity-90"
                        style={{ backgroundColor: selectedAI === 'claude' ? '#f97316' : selectedAI === 'chatgpt' ? '#10b981' : '#3b82f6' }}
                      >
