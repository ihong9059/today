...
// ARM cycle counter for precise timing
...
void send_grb(uint8_t r, uint8_t g, uint8_t b, uint64_t freq) {
    send_byte(g, freq);
    send_byte(r, freq);
    send_byte(b, freq);
}
...
int main() {
    ...
    // Get ARM timer frequency
    uint64_t freq = get_freq();
    printf("ARM timer frequency: %llu Hz\n", (unsigned long long)freq);

    printf("=== WS2812 with Cycle Counter ===\n\n");

    reset_led();
    printf("RED...\n");
    send_grb(150, 0, 0, freq);
    reset_led();
    sleep(3);

    reset_led();
    printf("GREEN...\n");
    send_grb(0, 150, 0, freq);
    reset_led();
    sleep(3);

    reset_led();
    printf("BLUE...\n");
    send_grb(0, 0, 150, freq);
    reset_led();
    sleep(3);
