...
static struct sw_uart tx2;  /* SPI1 MOSI = P0.15 (J28 Pin 2)  → RS485 */
static struct sw_uart tx3;  /* SPI2 MOSI = P0.22 (J28 Pin 7)  → Debug */
static struct sw_uart_rx rx2;  /* GPIO INT on P0.02            ← RS485 */
...
int main(void)
{
	...
	sw_uart_write_str(&tx3, "TX1/RX1=9600  TX2=9600 (SPI1, 1 MHz)  RX2=9600  TX3=115200\r\n");
	sw_uart_write_str(&tx3, "TX1=P0.11 RX1=P0.13 TX2=P0.15 RX2=P0.02 TX3=P0.22\r\n");
	...
	while (1) {
		...
		/* Diagnostic: RX2 ISR stats on TX3 every 5 seconds */
		if ((counter % 5) == 0) {
