...
## 4. 야외 노드 아키텍처 2분기 (CubeCell 등재 후 — lora-claude cascade 002 흡수)

신규 보드 **Heltec CubeCell HTCC-AB01** (ASR6501 = PSoC4000 M0+ + **SX1262** 통합 SiP, 온보드 솔라/배터리 관리 + USB CP2102 flash) 검토 결과, 야외 노드 설계가 2경로로 분기:

| | 모듈형 (한림 양산 검증) | 통합형 (CubeCell) |
|---|---|---|
| 구성 | nRF52832 + 외부 E22(UART) + 전원체인(CN3791+HT7333) | 단일 보드(MCU+SX1262+솔라/배터리 내장) |
...
→ **전략**: 야외 무인 LoRa 노드 = **"통합형(빠른 PoC) → 모듈형(양산)" 2단계**. 양쪽 다 SX1262라 기술 자산(주파수·SF·결선·전원 SOP) 재사용 가능, 벤더 락인 낮음. 단 양산 신뢰성·인증 중시 시 RAK 계열(RAK3172=STM32+SX1262)도 검토 가치 ([[gaps]] 하드웨어 선정 함정 참조).
