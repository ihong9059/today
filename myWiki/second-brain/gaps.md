...
## 2026-09-22 — 위시캣 파트너 등급(PRIME·PRO·BOOST) 미확인 = 고액 프라이빗 접근 반복 봉쇄 (11회째 병목) ⭐⭐ (wishket-claude 카드 `2026-09-11-001`·`2026-09-18-002` 흡수)

**함정**: 위시캣 고액 공고 중 **PRIME·PRO·BOOST 파트너 등급 한정** 프라이빗 건이 반복 등장하는데, UTTEC의 현재 등급이 미확인이라 **열람·지원 가능 여부 자체를 판단 못 한 채 3회째 이월(9/8 #158241·158284부터 누적 11회 관찰)**. 봉쇄된 실제 규모 = #158422 골프 플랫폼 **2억** + #158564 블랙박스 3,500만 + #158314 2,000만 + #158284 폐쇄망 LLM 9,000만 등 — **억대 파이프라인이 등급 미확인 하나로 접근 불가**. 검토 회차마다 ⚠️로 재관찰되며 데이터만 쌓이고 행동은 0.

**회피책**: **사용자 직접 행동(👤) — 위시캣 로그인 → 마이페이지에서 현재 파트너 등급 1회 확인**하면 해소되는 단발 병목. 등급 확인 후: ① 접근 가능하면 고액 프라이빗 선별 지원 ② 불가하면 등급 승급 조건(누적 실적·평점) 확인 → 승급 로드맵. **오늘 할일 상시 리마인드 유지**(11회 반복 = 자동 관찰이 행동을 대체 못 하는 구조적 이월). → [[위시캣활동]] 2026-09-22 § + 작업보고서 #2.
...
**함정**: 사용자가 실제 보유한 강점 자산(데스크톱 앱·220V 전열·Zigbee 등)이 me.md/양산제품 인덱스에 미등재 → **위시캣 지원서 초안이 구조적으로 약하게 나감**. me.md 항목 6·7·8·9·10·12에서 **반복 누락 6회** 박제. 특히 **데스크톱 앱을 약점 축으로 오분류**(6번째)했다가 #157744 계기 강점으로 정정, **220V 전열/히터 실적**은 #157726 최강 근거인데 구체 사실 미박제(4번째 반복).

**원인**: 지원서 작성 시점에 "이 요구에 맞는 우리 실적이 있나?"를 **사용자에게 선질의하지 않고 인덱스만 신뢰** → 인덱스 공백 = 자산 없음으로 오판. 실제로는 사용자 확인 시 대부분 "직접 양산/개발 경험 있음"으로 확답.

**회피책 (SOP)**:
1. **PC앱·GUI·데이터로거·전열/히터·무선(Matter/Thread/Zigbee) 요구 외주 지원 시 → 사용자 세부 실적 선질의 필수** (제품명·수량·인증). "인덱스에 없음 = 자산 없음" 단정 금지 ([[strengths]] 자산확인 SOP = 약점 선언 전 사용자 확인과 동일 문법).
2. 확답 자산은 **즉시 [[양산제품]]/me.md 인덱스에 등재** → 다음 지원서 자동 재사용(반복 누락 차단).
3. 미확정 실적(220V 전열 세부)은 **"사용자 확정 후 반영" 표기** — 과대주장 차단 ([[feedback_no_fabricated_user_data]] 준수, 수치·고객명 환각 금지).
...
**함정**: 일본 협력회사(受託 발주자) 접촉 채널의 다수가 **문의폼 PDF 첨부 불가**. 소개자료를 PDF로만 준비하면 1차 접촉 자체가 막힘 — "발송 킷 = PDF"라는 한국식 전제가 일본 문의폼에서 무효.

**회피책**:
1. **일본어 소개 랜딩(라이브 URL)을 소개자료 대체 채널로 우선 준비** — 문의폼 본문에 URL 삽입 = PDF 없이 회사·실적 전달. (GitHub Pages HTTPS 배포로 5社 발송본 완성.)
2. PDF는 **회신 도달 후 첨부**(2단계). 1차 = URL, 2차 = 상세 PDF.
3. 실 식별정보·발송킷·PDF는 **로컬 전용(gitignore)**, vault 추적은 placeholder 템플릿만 (마스킹).

→ 단일 출처 = [[일본-시장]] § 2026-08 受託 도급 채널 + wishket vault 로컬 발송킷. [[ai-direction]] § 2026-08-07 판단 로그.
