...
type: entity
created: 2026-05-07
updated: 2026-06-17 _inbox 흡수 — R48 Phase 5.4 IMU 제스처 양산 파이프라인 + R49 Phase 1 비지도 이상탐지(CWRU isolation forest F1 0.995, 예측정비 신축) + 중력-방향 shortcut 버그 (ondevice-claude 카드 2026-06-14-002, 결정 53) / 이전 2026-06-04 R50 Step 1~3 + sensor 12 모듈 + AI 매트릭스 단일 출처 + factory-rpi4 6번째 ssh + 워크플로우 §0.4 + R48 Path C (카드 005+006+007, 결정 41~43)
status: ✅ 1차 mandate 전환 완료 (5/17) / **v2.5 7/7 + v2.6 4/4 + v2.7 4/4 + v2.8 6/6 + v2.9 R36 ✅ = 6/6 mandate 모두 종결** (5/27) / **R37 후속 ✅ + R38 후속 ✅** (5/28 mandate v2.10 R38 SDRAM+QSPI 정량 실증, mywiki 권장 #4 70% 완성) / **14/14 보드 100% 완성** / 5계열 매트릭스 + Cortex-M tier 최강 + 3-tier 메모리 정량 실증 / **응용 진입 직전 마지막 측정 mandate 완성** + R37/R38 정정·실증 박제 = R&D 신뢰성 자산 강화 — 사용자 결단 (b 영업 데모 / c 양산 / R41 firmware 통합 진입)
tags: [vault, On-Device AI, 보드한계모델, AI FanStick, ESP32-S3, ESP-DSP, ESP32-C6, RISC-V, Xtensa, ARM, ARM-A, NEON, dotprod, Cortex-M4F, Cortex-M7, CMSIS-NN, Nordic, STM32, STM32H745, Zephyr-cross-vendor, Stage4, LAN-path, 정지선, multi-agent, ondevice-claude, mandate-v2.5-종결, mandate-v2.6-종결, mandate-v2.7-종결, mandate-v2.8-종결, mandate-v2.9-종결, mandate-v2.10-R38, 6mandate-모두종결, R37-positive, R37-정정사이클, baseline-artifact-정정, STM-15-INFO-emit-cache, STM-16-fmc-sdram-Kconfig, 17x-가속, SLM-적재, LoRA, fast-adam, KWS, selective-personalization, Hybrid-SoC, 5계열매트릭스완성, 6-negative-finding, 6조건곱, 51빌드함정, vectorizer-정책, Cortex-M-tier-최강, LiteRT, Jetson-Super, asymmetric-multiprocessing, QSPI-128MB-SFDP, 3tier-메모리, SDRAM-penalty-zero, Phi-2-적재-실증]
...
> **2026-06-17 _inbox 흡수 — R48 Phase 5.4 IMU 제스처 + R49 Phase 1 비지도 이상탐지 ⭐⭐⭐⭐** (ondevice-claude 카드 #2026-06-14-002):
> **edge AI 모델 클래스 지형도 확장 — 지도 분류 3종(MNIST/KWS/gesture) → 비지도 이상탐지 신규 1축**. ① ⭐⭐⭐ **on-device 비지도 이상탐지 역량 확보** — CWRU 산업 표준 베어링 진동에서 isolation forest 정상만 학습 → 결함 **F1 0.995**. 기존 검증이 모두 지도 분류였던 데서 **예측정비(predictive maintenance) 산업 패러다임**으로 확장. ② **IMU 제스처 end-to-end 양산 파이프라인** — 실측 수집(Pi+MPU smbus2)→mixed 학습→INT8→Cortex-M4F(pca10056) 배포→실시간 시연(idle/shake 100% / circle 67% / wave 33% / 6.14ms).
> **신규 함정 3건 (gaps prepend)** — ⭐ **중력-방향 지름길(shortcut) 버그** (제스처를 동작별 다른 방향으로 들고 수집 시 모델이 움직임 아닌 중력 방향을 라벨 단서로 학습 → **held-out 100%여도 on-device fail**, 교정=회전 증강 또는 per-window DC 제거. "검증 자체를 검증하라"의 강력한 사례) + 회전 증강↔INT8 양자화 손실 tradeoff(증강이 activation 범위 확대 → INT8 -10.8pp, per-channel 양자화 완화) + 저가 IMU die 변동(같은 "MPU-9265" 마킹도 die WHO_AM_I 0x70/0x74 배치별 상이, 데이터=추론 HW 일치 권장).
> **신규 decision (ai-direction § 결정 53)** — onDevice edge AI 모델 클래스를 **지도 분류 + 비지도 이상탐지 2축**으로 확장. 산업 예측정비(Stage 4) = 비지도 이상탐지 1순위 패러다임으로 영업 narrative 추가. 다음 센서 순서: MAX31865/ADXL345 이상탐지(R49) → pca10040(64KB 극한) 양산성.
> **매칭 패턴 ★** — **산업 예측정비 = 한국기계 Stage 4 + 산업 영업 직결** (CWRU isolation forest F1 0.995 = "검증된 산업 표준 알고리즘 single-chip $3~5 양산" narrative, 위시캣 산업 고객 / 강사양성 산업 AI 교육 시너지) + **중력-방향 버그 = 교육 자산** ("AI 검증의 함정 = 데이터 누수/shortcut learning" 강사양성·교육 콘텐츠 직접 활용).
> **자세히**: [[ai-fanstick]] § IMU gesture 양산급 + [[ai-direction]] § 결정 53 + [[gaps]] § 2026-06-17 onDevice 3건 + [[strengths]] §20 비지도 이상탐지 + [[factory]] § 예측정비 narrative.

> **2026-06-04 R50 Step 1~3 완성 + sensor 12 모듈 + AI 매트릭스 단일출처 + factory-rpi4 6번째 ssh + 워크플로우 §0.4 + R48 Path C ⭐⭐⭐⭐⭐** (ondevice-claude 카드 005+006+007 megasession 흡수):
> **(R50 Step 1+2+3 본질 완성, work-end #1+#2+#3 누적 ~11.5h, 카드 005)** ⭐⭐⭐⭐ **Path D 산업 HMI 손글씨 인식 demo 본질 완성**. (Step 1+2) R50 INT8 quantization PyTorch FP32 99.41% → INT8 99.37% (-0.04pp, per-tensor symmetric INT8 + p99 strategy + Q31 multiplier/shift). 105K params → 105KB. mnist_cnn.c (CMSIS-NN conv + plain C FC g-3 fix). **STM32H745 + Plain C ≈ CMSIS-NN (Cortex-M7 특이성)** — plain C 8.13ms ≈ CMSIS-NN 8.28ms (M7 dual-issue + L1 cache 자동 vectorize). Cortex-M4F (R46 pca10056)와 본질 다름 → 결정 43 (chip-specific 매트릭스). (Step 3) R50 LCD UI carrier mnist_ui_project/ (Zephyr 4.3.99 LTDC overlay + PLL3 + display API + input subsystem + 5×7 font + flag deferral pattern). **UM2488 Rev 10 ground truth** 박제 (28-pin LTDC pinmap + Arduino Uno V3 pinout + STMod+ 20-pin pinout). **본 vault 첫 Zephyr stm32h745i_disco LCD/touch overlay 박제** (Zephyr 4.3.99 stock 미지원 → carry source).
> **신규 함정 4건 (gaps prepend)** — R50-1 ⭐⭐⭐⭐ `arm_nn_vec_mat_mult_t_s8` STM32H7 비결정 saturate (CMSIS-NN chip×library×toolchain 매트릭스 검증 필수, plain C 우회) + STM-7 v2 PK7=LCD_DE / PD7=LCD_DISP (R36 옛 박제 정정) + R50 LCD overlay carry pattern + R50 Stack overflow callback 직접 inference (flag deferral pattern, CONFIG_MAIN_STACK_SIZE=16384).
> **(sensor 라이브러리 11→12 모듈 완성 + AI 매트릭스 단일 출처 mandate, 카드 005+006)** ⭐⭐⭐ **신규 sensor 정식 박제**: FT5336 (Focaltech 5-point capacitive touch, I2C 0x38, STM32H745I-DISCO onboard, R50 Step 3-B 검증, Path D 손글씨 input carrier) + INMP441 (TDK InvenSense I2S MEMS 디지털 마이크, BOM $2, R44 KWS/R47 ESP-NN/R50 multi-modal 표준 carrier, cross-platform STM32/ESP32/Nordic) + `_STM32H745_EXPANSION/` (Arduino Uno V3 + STMod+ pinout + sensor 매핑 + I2C bus 공유 함정 + Zephyr dts overlay 패턴). **sensor AI 매트릭스 단일 출처 mandate** (`sensor/AI_매트릭스.md` 491 lines, 11 sensor × 2~3 AI model × 14 보드 최저선 매핑, 최저선 보드 3축 정의: RAM hard wall + accuracy ≥ acceptable + latency ≤ 양산 실시간). 검증 carry 박제값 (절대 신뢰): R18 3.23× / R44 KWS 9.91ms 75% / R46 9.26ms / R47 1.06× / R50 8.13ms 100%. **외부 module 7 카테고리 카탈로그** (`sensor/_추가_module_후보/` 8 파일 1,466 lines, 40+ module: 압력/거리TOF/환경/모션/광학/산업/농업, 일괄 발주 BOM $100~150 핵심 6).
> **(R48 Path C Phase 1~4 + factory-rpi4 + 워크플로우 §0.4 + INA219, 카드 007)** ⭐⭐⭐ **factory-rpi4 = 6번째 ssh 머신** (Tailscale 100.109.84.79, Pi 4 Model B Rev 1.5 Cortex-A72 × 4 4GB, asimddp 없음, Debian 13, gcc 14.2 aarch64, PyTorch 2.12.0+cpu venv `--system-site-packages` ~150MB). 본 vault 6번째 ssh 머신 (uttecMac/uttecRpi3/4/5/zero + factoryRpi4). 3중 역할: production QC + 데이터 수집 + 작은 모델 학습/INT8 quantize. **INA219 (12번째 sensor)** TI digital I2C 전류·전압·전력 monitor ±0.5% BOM $3. AI 응용 3개 (battery SoC pca10056 / 가전 NILM pca10040 / 산업 모터 anomaly autoencoder pca10056). **워크플로우 §0.4 표준** (단일 출처) — 원격 학습 + 최저선 deploy 2단계, 6 step 표준, 검증 carry 6 instance (R18/R44/R46/R47/R50/R48 Path C 100% Phase 1~4). **R48 Path C 결단**: 사용자 보유 MPU-92.65 cable 연결만 + 고정 안 됨 → Path B 보류, Path C (합성 dataset) 진입. 결과 100% accuracy + 3.17KB INT8 + delta 0pp. 응원봉 wearable form factor 고정 후 Path B 후속 결단 carry (Transfer learning carry value: Path C pretrain → Path B fine-tune).
> **신규 함정 5건 추가 (gaps prepend)** — I2C bus 충돌 AHT21 (0x38) ↔ FT5336 (0x38) onboard touch (별도 I2C bus 필수) + numpy flatten 순서 = PyTorch channel-first 필수 (random baseline 25% 함정) + WHO_AM_I 5종 분기 (MPU-9265 0x74 die 변형 신규) + PEP 668 Debian 13 → `python3 -m venv --system-site-packages` + scp wildcard `{a,b}` 미동작 (`*` 사용).
> **본 vault 박제 영향** — Path D 산업 narrative 정량 완성 (BOM $30 / 6~7ms / 100%) + Stage 4 신규 응용 path 첫 정량 검증 + 영업 cascade 진입 가능 (uttec-vault / business/entities/AI_FanStick.md / 한국기계 외 신규 영업 후보 발굴). 본 vault 11 sensor 중 **9개는 pca10056 (Cortex-M4F 256KB) 이내 양산 가능** = AI FanStick 차세대 BOM에 거의 모든 sensor 추가 가능 (영업 narrative 결정타).
> **다음 cascade carry** — R50 Step 4·5 / R48 Phase 5 (응원봉 form factor 고정 후 Path B fine-tune) / 외부 module 일괄 발주 결단 / business/entities/AI_FanStick.md Path D 기술 근거 추가.
> **자세히**: [[ai-fanstick]] § Path D 정량 + Path B-4 + sensor cluster $50 + [[ai-direction]] § 결정 41~43 + [[gaps]] § 9 함정 + [[strengths]] § 13 carrier 역량 + [[2026-06-04_sensor-AI-매트릭스-단일출처-mandate]] (신규 thought).

> **2026-06-03 R50 신설 흡수 ⭐⭐⭐** (ondevice-claude 카드 #2026-06-03-001):
> **R50 STM32H745I-DISCO LCD touch 손글씨 MNIST CNN inference PoC 진입** — R48 IMU carry (MPU6050 미준비 → 내일), 사용자 신규 방향 결단으로 본 vault 첫 LCD+touch+CNN 통합 PoC 진입. 본 vault `프로젝트_보드한계모델_v2.10/Round50_STM32_Touch_MNIST/` skeleton 작성 (00_실험계획서.md 포함). Cortex-M7 480MHz + CMSIS-NN으로 MNIST CNN INT8 inference + 실시간 touch UI 동시 통합.
> **Step 0 PC sanity MNIST CNN INT8 ✅ 99.41% PASS** — LeNet-5 변형 ~105K params / INT8 ~105KB / Cortex-M7 fit / 99.41% PC sanity (artifacts/model_fp32.pt 학습 완료). 보드 INT8 ≥95% 목표 (Step 1~5 carry: R46 calibrate_int8.py carry → MNIST 변형 + INT8 양자화 + CMSIS-NN port → STM32 BSP touch + LCD canvas → stroke normalize + inference → 99_결론 + 영업 cascade).
> **AI FanStick Path D 산업 응용 path 신설** ⭐⭐⭐ — K-POP 외 첫 응용 확장. BOM ~$30 / Cortex-M7 + LCD + touch + AI 단일 chip = 키오스크 / 스마트팩토리 HMI / 의료 input pad B2B narrative. 기존 Path A $25 / B $31 / B-2 $16 (모두 K-POP) 외 산업 영역 입구.
> **5계열 AI 가속 매트릭스 본 vault 응용 진입 단계 도달** — LX7 ESP-DSP / M4F + Cortex-M7 / esp-nn / ARM-A NEON / NPU 5계열 측정 완료 (Round 1~46) → R50 = 통합 PoC 응용 단계 첫 사례 (측정 → 매트릭스 → 통합 → 응용 4단계 깊이 진화).
> **신규 함정 2건 (gaps prepend)** — bash backslash Windows path escape (`mkdir C:\r50_proj` → `Cr50_proj` invisible-char dir 부산물) + PyTorch 환경 박제 (Python 3.13 sandboxed vs 3.14 Programs, `where pip` 사전 확인). cross-shell 환경 함정 인벤토리 추가.
> **매칭 패턴 발견 (thoughts 신규)** — 위시캣 키오스크/HMI/의료 input pad 일감 cluster (자동 매칭 SOP 추가) + 강사양성 Day 5 사례 후보 (R26 KWS + R34 Hybrid SoC + R36 STM32H745 + R50 LCD touch) + uttec-vault 위치 확인 carry (메모리 5/23 박제 후 변경 가능, 사용자 확인 후 Day 5 cascade 재개).
> **자세히**: [[ai-fanstick]] § Path D + [[2026-06-03_R50-touch-mnist-path-D-산업응용]] (신규) + [[ai-direction]] § 결정 32~34 + [[gaps]] § bash backslash + PyTorch 환경 + [[위시캣활동]] § 키오스크 cluster.

> **2026-06-01 R44/R45/R46 megasession 흡수 ⭐⭐⭐⭐⭐** (ondevice-claude 카드 3장 일괄 — #2026-05-30-001 R41 Path B SW carry + R42 신설 + #2026-06-01-001 R44 INT8 sanity PASS + esp32s3 build PASS + #2026-06-01-002 R44 양산 verdict + R45/R46 CMSIS-NN 가속 검증):
> **(R41 Path B SW carry, 5/30)** vault Zephyr 4.3.99 WM8994 codec driver 신규 작성 (input + output + register dump + cold-start 4 함수) + 8단계 STAGE emit gate framework. ST sample (SAI_AudioPlayback + BSP) 활용으로 보드 audio output **100% 검증** (sin wave 500Hz + AUDIO PLAY audible). **R42 신설** — `Round42_STM32H745_KWS_평가/` 7 파일 + 4 폴더 skeleton, mic 우회 path (kspon_kw 한국어 KWS dataset 23,731 sample 활용). vault AI on-device app 본격 진입 첫 Round. STM-17~21 신규 함정 5건.
> **(R44 진입 + INT8 0pp, 6/1 #1)** ⭐⭐ **INT8 quantization 정확도 손실 0.00pp** — MLP 1024→128→8 symmetric per-tensor INT8: PC float32 75.0% (120/160) → PC INT8 simulation 75.0% (120/160), per_keyword 패턴 동일 (down/stop 85% / right 55%, R26 baseline 보존). **esp32s3 Path B 검증 통과** — esp32_kws.bin 507KB, Flash .rodata 332KB + .text 96KB / DRAM 13KB (520KB SRAM 중 2.5%) / IRAM 16KB → **PSRAM 불필요** = 부록 D Path B 양산 표준 정량 검증. 신규 함정 #17 bootloader build.ninja race / #18 CMakeTestCCompiler 우회 / #19 Initialize-Idf.ps1 fail → export.ps1 대체.
> **(R44 양산 verdict + R45/R46, 6/1 #2)** ⭐⭐⭐ **R44 3-board KWS 매트릭스 ✅ 양산 verdict (esp32s3 메인 + pca10056 BLE 결합) + R45 CMSIS-DSP negative + R46 CMSIS-NN full FC 가속 가설 정확 재현** (R18 carry 3.23× → 본 모델 3.14× ✅). **본질 finding**: "pca10056 + CMSIS-NN ≈ esp32s3 plain C 동급 latency" ($15 M4F + 가속 = $5 LX7 plain C). **5중 일치 75%** (R26 PyTorch + R42 STM32 + R44 esp32s3 + R44 pca10056 plain C + R46 CMSIS-NN) — 모델 transfer + INT8 quantization + 보드 implementation + library port 모두 손실 0pp 검증.
> **신규 가속 매트릭스 row** — Cortex-M4F + CMSIS-NN full FC = **3.14× 가속** (R46 검증, R18 carry 일반화) / Cortex-M4F + CMSIS-DSP dot only = **1.077×** (R45 negative finding C: plain C가 gcc 12.2 `-Os`에 SMLAD vectorize 잘 됨 + API 단위 가속 본질 = fused operation vs separate).
> **gotcha R46-nrf1 ⭐⭐** — CMSIS-NN `arm_fully_connected_s8` filter_dims layout mismatch (API expects [in_dim, 1, 1, out_dim] = in_dim major, vault carry weights = [out_dim × in_dim] row-major → API 잘못 해석). 진단 단서: "accuracy random (~1/N_classes) + latency 정상" → first-check filter layout. 우회: `arm_nn_vec_mat_mult_t_s8` ([out × in] row-major 직접 호환, 동등 가속). 영구 자산 (미래 CMSIS-NN port 즉시 우회). gotcha R45 — `CONFIG_CMSIS_DSP=y` 만으로 부족, `CONFIG_CMSIS_DSP_STATISTICS=y` 명시 필수.
> **BOM Path B-2 영업 신설** ⭐⭐ — Path A $25 / Path B $31 + 신규 **Path B-2 pca10056 단독 ~$16** (nRF52840 단독, BLE5 + KWS 단일 SoC, SLM 없음) — K-POP 저가형 / OEM / 매스마켓 옵션. CMSIS-NN port 표준 = `arm_nn_vec_mat_mult_t_s8` 채택 (향후 모든 R47+ 모델 port default).
> **자세히**: [[ai-fanstick]] § BOM 3-path / [[2026-06-01_R44-3board-verdict-CMSIS-NN-fused]] (신규) / [[build-gotcha-inventory]] § esp32 #17~19 + R46-nrf1 + R45 CMSIS-DSP / [[gaps]] § 양산 path 함정 / [[ai-direction]] § BOM 3-path + 가속 가설 검증 framework.

> **2026-05-29 R41 Path A 본격 진입 흡수 ⭐⭐⭐⭐⭐** (ondevice-claude 카드 #2026-05-29-002):
> **vanilla Zephyr STM32H7 DMIC 정식 지원 0 → 본 vault custom patch chain 11.5/12 단계 PASS** — 5/29 work-start #2 ~6시간 누적, samples/drivers/audio/dmic/boards STM32 0 보드 + dmic_stm32 driver 0건 확인 (옛 박제 "path A 1~2일 작업" 가설 정정), 본 vault custom Zephyr patch chain (binding 1 + driver 8 + overlay v3 + main.c) carry로 11.5/12 단계 PASS 검증.
> **SAI4 register 결정타 진단** ⭐⭐⭐ — RCC.APB4ENR bit21 = 1 (SAI4 clock enable) + PDMCR = 0x00000101 (PDMEN + CKEN1, PDM mode 활성) + **ACR1 = 0x000B1281 (SAIAEN=1 + DMAEN bit17 = 0)** ❌ HAL BDMA path 결함 결정타. ACR1 write protection 때문에 disable→set→enable 시퀀스도 변화 0 → 다음 세션 ~수시간 patch.
> **PC1+PE2 2핀 Ethernet 양방향 충돌 확정** ⭐⭐⭐⭐ — Zephyr default `stm32h745i_disco m7.dts` line 131~155: PC1 = `eth_mdc_pc1` (mdio node) + PE2 = `eth_txd3_pe2` (mac node) → Ethernet MII 모드 활성 시 MEMS DMIC 절대 불가 (SW `status="disabled"` 필수). 5/28 박제 "PE4 ≠ PC1 충돌 없음" → 5/29 박제 "PC1+PE2 양방향 확정" 정정.
> **시나리오 E E1/E2/E3 분리 박제** ⭐⭐⭐ — E1 (Voice/KWS 단독, ETH X), E2 (LAN AI 산업 노드, DMIC X), E3 (양립, hw modification 필수 = Phase 4 ship 옵션 별도). 영업 라인업 = E1 + E2 명시 카탈로그.
> **신규 STM 함정 5건 (STM-17~21)** — R41-2 Zephyr binding 자기모순 / R41-3 SRAM4 nocache 누락 (BDMA SRAM4 D3 domain D-cache stale) / R41-4 i2s_stm32_sai BDMA 미지원 (GPDMA만, driver fork 필요) / ACR1.DMAEN write protection (RM0399 누락 spec) / BDMA SRAM4 D3 domain buffer 제약 (AXI SRAM access silent transfer 0). STM 함정 누적 16 → **21건**. cross-vendor 누적 51 → **56건**.
> **Stage 4 시나리오 E1 영업 자산 회복 path 확정** ⭐⭐⭐ — 옛 박제 (5/24): "stm32h745 single chip + DMIC voice command 본질 불가능" → 새 박제: ✅ 11.5/12 PASS + 본 vault custom Zephyr patch chain R&D 자산. 다음 세션 ACR1.DMAEN write protection 우회 patch 시 R41 본격 종결.
> **Zephyr upstream PR contribution carrier** ⭐⭐ — 본 vault custom patch chain (i2s_stm32_sai BDMA aware + mpxxdtyy STM32 H7 정식 지원) = Zephyr upstream PR 기여 후보. 위시캣 / 강사양성 측 Zephyr/STM32 customer engagement 발생 시 본 patch chain 카드 가치 carry.
> **자세히**: [[stm32h745-disco]] § R41 absorb + [[2026-05-29_R41-Path-A-본격-진입-stm32h745-SAI4-BDMA]] + [[ai-fanstick]] § 시나리오 E1 회복 + [[uttec-stage-package]] § E1/E2/E3 분리 + 6 항목 우위 + [[build-gotcha-inventory]] § STM-17~21.

> **2026-05-28 R38 SDRAM+QSPI 정량 실증 흡수 ⭐⭐⭐⭐** (ondevice-claude 카드 #2026-05-28-002):
> **mandate v2.10 R38 진입** — 5/28 오후 14:00~18:30 단일 세션, mywiki 권장 #4 (SLM Phi-2 mini Q4 QSPI XIP) 부분 정량 실증 4 Phase 완료 (Phase A SDRAM read latency / B QSPI throughput / C D-cache / D SDRAM weights forward).
> **QSPI 64MB → 128MB 박제 정정** ⭐⭐ — SFDP 실측 결과 옛 박제 "Macronix MX25LM51245G 512Mbit"는 dts upstream 추정 → 실제 **MX66LM1G45G 1Gbit 추정 (SFDP detect)**. 총 Flash XIP **129MB** (internal 1 + QSPI 128). Zephyr upstream `boards/st/stm32h745i_disco/...dts` `DT_SIZE_M(64)` + ST UM2381 모두 upstream 정정 필요.
> **Phi-2 mini Q4 50MB 적재 정량 실증** — 옛 가설 "50~60MB 가능" → ✅ boot 3.22s / throughput 15.51 MB/s. multi-SLM capacity 2× (129MB / 50MB) 적재 가능.
> **3-tier 메모리 모델 정량 실증** — DTCM 128KB (1.0×) + SDRAM 8MB (1.28× slow only) + QSPI 128MB (15.51 MB/s). D-cache 효과 SDRAM 4.19× / DTCM 2.82×.
> **Phase D 영업 결정타** ⭐⭐⭐ — 857K params MLP SDRAM 배치 forward 10.1ms (R36 DTCM baseline 대비 latency ratio 18.14× < param ratio 20.29× = **11% 더 효율적**). "SLM SDRAM 적재 = DTCM 적재와 거의 동등 효율" 영업 카피 신설.
> **STM-16 신규 함정** ⭐ — Zephyr stm32 fmc_sdram driver Kconfig 필수 (`CONFIG_MEMC=y + CONFIG_MEMC_STM32_SDRAM=y`). dts node `status="okay"` 만으로는 SDRAM access 시 Imprecise BUS FAULT. STM 함정 누적 15 → **16건**. cross-vendor 50 → **51건**.
> **Stage 4 시나리오 E 5 항목 우위** (4 → 5) — R38 정량 실증으로 "3-tier 메모리 + Phi-2 50MB 적재 + SDRAM penalty zero" 추가.
> **본 vault `프로젝트_보드한계모델_v2.10/Round38_STM32H745_SDRAM_QSPI/` 박제 완료** + 5 commits local (5d31873 → 25ddf50). R41 진입 결단 시점 (mywiki 권장 #1 R35 한국어 KWS firmware 통합).
> **자세히**: [[stm32h745-disco]] § R38 absorb + [[2026-05-28_R38-stm32h745-SDRAM-QSPI-3tier-메모리-실증]] + [[ai-fanstick]] § 시나리오 E 5 항목 우위 + [[uttec-stage-package]] § 시나리오 E 박제 정정 + [[build-gotcha-inventory]] § STM-16.
...
> **메모리 4-tier 정정** — QSPI Flash 16MB → **64MB** (Macronix MX25LM51245G) / 총 RW RAM **9.2 MB** / 총 Flash XIP **65 MB** → **GPT-2 mini / Phi-2 mini Q4 (50~60MB) 적재 가능** (QSPI XIP read-only).
> **R36 신규 gotcha** — sweep race fix 단일 cell 패턴 (mass erase → port.Open + DiscardInBuffer → 300ms 대기 → reset → monitor) + CNN 64 monitor 부족 진단 (96초 bench, 150s monitor에서 정상 emit).
> **본 vault `프로젝트_보드한계모델/` 6/6 mandate 모두 종결**: v2.4 + v2.5 + v2.6 + v2.7 + v2.8 + v2.9 → **응용 진입 직전 마지막 측정 mandate 완성**.
