...
type: entity
created: 2026-06-13
updated: 2026-09-09 (_inbox 흡수 — lora-claude 카드 `2026-09-06-001` request: **드론군무 LoRa 응용 검토 트랙 신설 → 사업방향 탐색 트랙 등록**(옵션 B "비상 안전망 전용 채널" 포지션, 조대홍 client 협의 게이트, KR920 EIRP/LBT 정식 조사 전 확정 금지 → 결정 65) + **vault log 37일 cascade**(조명 단말 현장설정 v2 OTA 제거·golfWater CSMA/CA+배터리ADC+LTE GW 방향A·한림 g5 pump 병렬 쌍둥이 SWD 복제·**필로스CC 조명 교체 견적 2,497만+사장 보고용 시스템 교체 설명서** → [[필로스cc-조명제어]] entity 신설) / 이전 2026-08-02 (_inbox 흡수 — lora-claude 카드 `2026-07-26-001` done: 누수 자동 알림 룰 기술 로드맵 평가 완료 — 연산위치=RPi golf_bridge 후처리(노드 펌웨어 무변경)·우선순위=양산 단계(관수 상태 계측 선결)·"SW 로드맵" 영업 문구 유지 정당. 회신 불요(done) / 이전 2026-07-16 (_inbox 흡수 — lora-claude 카드 2장: `2026-07-14-001` done(노드수 11 확정 = 구 14 구설계, pond 2보드→1보드 addr 21·61·81 폐지 / 솔라 12mA·권장패널 5~10W 확정·자립일수 검증중) + `2026-07-14-002` SOP request(5국면 표준 작업계획서 → 영업 신뢰 자산 흡수 YES) → 결정 61 + gaps(설계변경↔영업자산 drift) + 영업자산 6파일 14→11 정합 + done ack 2건 / 이전 7/9: _inbox 흡수 — lora-claude 카드 1장: 공용 시스템 보드 = 수조+조명 단일 HW 플랫폼(고정 코어 LoRa+릴레이+nRF52832+2.4G ESB + 선택 실장) → 제품라인 통합·"단일 플랫폼" 영업 강점, 결정 60 + gaps(조명 fail-safe 반대) + thought 1 + done 회신 / 이전 7/5: _inbox 흡수 — lora-claude 카드 2장: BLE↔LoRa 브리지 게이트웨이 경량·무선화 monitor terminal end-to-end 실증(06-19) / 2.4G ESB 로컬링크 dual-radio 겸용 역량 확보(06-20) → 결정 58 + gaps 2026-07-05 + thought 1 + done 회신 / 이전 6/19: 통합펌웨어+BLE프로비저닝+한림 최종 아키텍처 3패턴(06-18) / 2.4G ESB 하이브리드 옵션(역량경계 ~30dB)+E22 generic SX126x 디코드 불가(벤더 락인)(06-19) / 이전 6/17: 카드 4장 megasession — 4종 모듈 검증+SPI 전환 경로(06-14) / 골프장 수조 제어망 프로토콜+8B frame 송수신 실증+배터리 모니터링(06-16) / CubeCell 스니퍼 done ack(06-15) / 합류 19th myWiki측 5단계 완결(06-13))
...
> **2026-09-09 _inbox 흡수 — lora-claude 카드 `2026-09-06-001` request (드론군무 LoRa 응용 검토 트랙) + vault log 37일 cascade** ⭐⭐:
>
> **(카드 09-06-001 request, 드론군무 LoRa 트랙 — 사업방향 탐색 트랙 등록)** ⭐⭐ — 2026-09-06 사용자 주도 신규 응용 탐색: 드론 군무(드론 라이트 쇼). lora vault `드론군무/` 검토 트랙 신설(00 개요·01 통신아키텍처·02 RTCM 상세·대외 설명자료 PDF). **조대홍님에게 설명자료 PDF 전달 → 실제 client 협의 예정 = 진행 게이트**. 기술 결론 5포인트: ① 드론쇼 통신 = LoRa 적성 영역(사전 안무+자율비행 = "적은 데이터를 멀리 확실하게", 시간동기는 GPS 시각 대체 → 레이턴시 무관) ② **golfLight 스택(1 master→N단말 broadcast·ACK재전송·fail-safe)이 드론 관제 C2 계층과 동형** = 개발량 최소 진입 ③ **권장 포지션 = 옵션 B "비상 안전망 전용 채널"**(기존 드론쇼 사업자와 경쟁 아닌 보완 — 주 통신 장애 시 독립 "전체 착륙" 채널, 안전 셀링포인트) ④ RTCM over LoRa 상용 선례(Emlid) 존재, 풀스택(옵션 A)도 N≤100 성립 ⑤ 🔴 **리스크 = KR920 EIRP/LBT(드론 고도 = 간섭 footprint 폭증, E22 22dBm 초과 가능성) 정식 조사 전 사업 확정 금지** + 항공 규제는 운영사 영역 선긋기. → [[ai-direction]] § 결정 65 + [[gaps]] § 2026-09-09 + [[2026-09-09_드론군무-LoRa-안전망채널-golfLight-C2-동형]] + [[조대홍]](협의 게이트). 인맥 접점(드론쇼·방제드론 수요) 역방향 공유 요청 = 사용자 결정 대기.
>
> **(vault log cascade 2026-08-16~09-08 — entity 37일 미반영분)** ⭐⭐ — ① **조명 단말 현장설정 v2(8/16, 사용자 확정)**: OTA 완전 제거, SWD flash + BLE 설정 + 2분창→main / 조명 웹버튼 무반응 = 무재전송 단발전송 충돌 규명·수정. ② **golfWater(8/16~18)**: golfLight 뼈대 재사용 master중심 수조제어 신규 구현 + sensor CSMA/CA 충돌회피 + 실 배터리 ADC + Thingy:91 X LTE 게이트웨이 방향 A 확정. ③ **한림 현장(8/19·9/2)**: g5 pump 병렬 쌍둥이 SWD readback 복제 실행·검증 + g7 pump 터미널(71) 진단·fail-safe 20초→10분(node 71 전용). ④ **필로스CC(9/1·9/8)** ⭐: 조명제어 개선 정식 견적서(대표이사 보고용, 총 24,970,000 VAT포함) + A/S 방문(배전반 동일 문제 재발, 5년+ 경과·소스/회로도 부재 = 원상복원 불가) → **1홀=1표준단말 교체 전환 결정, 사장 경영보고용 시스템 교체 설명서 A4 5p 산출** = 수주 협상 중 → [[필로스cc-조명제어]] entity 신설. "비표준 레거시 → 표준 플랫폼 교체" = 노후 설비 현대화 클러스터([[ai-direction]] 결정 64 패턴)의 자사 직영업 사례.

> **2026-07-26 _inbox 흡수 — lora-claude 카드 `2026-07-26-001` done (누수 자동 알림 룰 기술 로드맵 평가 완료)** ⭐⭐ (2026-08-02 흡수, in_reply_to `2026-07-12-001`):
>
> **(카드 07-26-001 done)** — 누수 자동 알림 룰 로드맵 5포인트 평가. 단일출처 = lora `수조제어_펌웨어/11_누수자동알림룰_기술로드맵.md`. ① **연산 위치 = RPi 대시보드(golf_bridge) 후처리 확정** — 판정 원자료(수위 `/level` CSV 3일 + 펌프 `/pump`)가 이미 RPi에 모임, **노드 펌웨어 무변경**(저전력·재flash 리스크 0). ② 판정 = `(펌프 OFF & 관수 OFF)` 구간 슬라이딩 관측창 Δ수위/Δt 임계 초과 지속. ③ 임계 = 1단계 고정(부피율 L/h) → 2단계 baseline 자가학습(CSV 이력으로 후향 산출, 추가 계측 불요). ④ 경보 = 대시보드 시각 + 서버 push(이메일/Telegram, 현 스택 재사용 비용 0). ⑤ **난이도·우선순위 = 양산 단계 착수 권장**(즉시 아님). **⚠️ 결정적 한계 = 관수(스프링클러) 살수 여부 미계측** → OFF 구간에 구멍 → "무오탐 완전 자동" 클레임의 **선결 조건**. **→ 영업 정합**: "누수 자동 알림 룰 = SW 추가개발 로드맵" 문구 **유지 정당**(현=육안 판단), 승격 조건 = 관수 상태 계측 확보 + 고정임계 MVP 배포. 기술 실현성 높음(RPi 후처리·자산 재사용), 유일 리스크 = 관수 미계측 오탐. done 카드 = **회신 불요**([[한림용인cc-고가수조]] § 누수 알림 / [[영업전략]] SW 로드맵 문구 근거).
