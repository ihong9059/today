# Multi-Agent Claude 협업 시스템 — 빠른 진입 가이드 (5 Claude)

> 새 Claude 세션이 한 번에 시스템 전체를 파악하고 즉시 합류 가능하게 만든 진입점.
> 본 가이드는 합의 일자: 2026-05-12. 갱신: 2026-05-16 (shield-claude 합류, 5 Claude로 확장).

## 핵심 한 줄

> **5 Claude(`mywiki-claude` / `revita-claude` / `ondevice-claude` / `n8n-claude` / `shield-claude`)가 각자의 프로젝트 폴더에서 `_inbox/pending/` 메일박스를 통해 비동기 협업한다. 사용자가 broker 역할을 하지 않아도 ingest·흡수·정합화가 자동.**

## 시스템 구성도 (5 Claude, 5/16~)
...
│  → 응답 카드 → revita / ondevice / n8n / shield 측 _inbox/      │
└────────────────────────────────────────────────────────────────┘
                          ↕ (비동기)
...
└────────────────────────────────────────────────────────────────┘
                          ↕ (비동기, 분산 호스트)
┌──── shield-claude (작업 위치: /home/uttec/project/shield/) ────┐ ⭐ NEW (5/16)
...
## 핵심 자산 (각 위치, 5/16~ 5 Claude)

| 위치 | 파일 | 역할 |
|---|---|---|
| `C:\todo\today\myWiki\_inbox\` | `PROTOCOL.md` | 5 Claude 동일 표준 |
...
| `/home/uttec/project/n8nUttec/_inbox/` (5/16~, Ubuntu) | `PROTOCOL.md` 사본 + `SYSTEM_GUIDE.md` 사본 + `pending/processed/` | n8n 측 메일박스 (분산 호스트) |
| `/home/uttec/project/n8nUttec/.claude/hooks/` | `check-inbox.py` | `SELF_ID="n8n-claude"` |
| **`/home/uttec/project/shield/_inbox/`** (5/16~, **RPi**) | **`PROTOCOL.md` 사본 + `SYSTEM_GUIDE.md` 사본 + `pending/processed/`** | **shield 측 메일박스 (분산 호스트)** |
| **`/home/uttec/project/shield/.claude/hooks/`** | **`check-inbox.py`** | **`SELF_ID="shield-claude"`** |
| **`/home/uttec/project/shield/.claude/skills/`** | **`work-{start,end}/SKILL.md`** | **multi-agent 통합 버전 + § 5-F always-send 룰** |
...
**적용된 셋업 패키지**: `today/obsidian/myWikiSetup/templates/` — 시나리오 D 세 차례 적용 (3·4·5번째 위키), 본 패키지의 메타 검증 강화 (분산 호스트 3 사례 누적: Windows 1 + Linux 2)
...
- **2026-05-16**: **n8n-claude 합류** — `/home/uttec/project/n8nUttec/` (Ubuntu 호스트) multi-agent 합류. 4 Claude 시스템으로 확장 (myWikiSetup 시나리오 D 두 번째 적용 사례 — 분산 호스트 Windows ↔ Linux 검증).
- **2026-05-16**: **shield-claude 합류** — `/home/uttec/project/shield/` (RPi Linux 호스트) multi-agent 합류. RPi shield 보드 응용 개발 (LoRa·RS485·RS422·MESH·I2C 센서·디스플레이). 5 Claude 시스템으로 확장 (myWikiSetup 시나리오 D 세 번째 적용 사례 — **분산 호스트 3 사례 누적**: Windows 1 + Linux 2). work-end SKILL § 5-F를 "always send absorb card" 강제 룰로 커스텀 (myWiki와 항상 자동 연결).
...
- `myWiki/second-brain/thoughts/2026-Q2/2026-05-16_shield-claude-합류.md` — 본 사례 (5 Claude 확장 + 분산 호스트 3 사례)
...
| 신뢰 검증 | 첫 사이클 4 카드 정상 완료 + hook 동작 검증 + 시나리오 D 3 사례 |
