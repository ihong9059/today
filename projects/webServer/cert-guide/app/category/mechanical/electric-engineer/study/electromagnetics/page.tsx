          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="flex gap-3 flex-wrap">
                <button
                  onClick={() => { navigator.clipboard.writeText(activeQuestion.prompt); window.open("https://claude.ai/new", "_blank"); }}
                  className="flex-1 min-w-[120px] py-3 rounded-xl font-medium transition flex items-center justify-center gap-2 text-white bg-orange-500 hover:bg-orange-600"
                >
                  🧡 Claude
                </button>
                <button
                  onClick={() => { navigator.clipboard.writeText(activeQuestion.prompt); window.open("https://chat.openai.com/", "_blank"); }}
                  className="flex-1 min-w-[120px] py-3 rounded-xl font-medium transition flex items-center justify-center gap-2 text-white bg-green-600 hover:bg-green-700"
                >
                  💚 ChatGPT
                </button>
                <button
                  onClick={() => { navigator.clipboard.writeText(activeQuestion.prompt); window.open("https://gemini.google.com/", "_blank"); }}
                  className="flex-1 min-w-[120px] py-3 rounded-xl font-medium transition flex items-center justify-center gap-2 text-white bg-blue-500 hover:bg-blue-600"
                >
                  💙 Gemini
                </button>
              </div>
              <p className="text-xs text-gray-500 mt-3 text-center">클릭하면 프롬프트가 복사되고 해당 AI가 열립니다</p>
            </div>
