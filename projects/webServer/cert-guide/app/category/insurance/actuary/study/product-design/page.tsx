...
const topics = [
  {
    ...
    questions: [
      {
        id: 1,
        question: '보험상품 개발 절차는?',
        answer: '시장조사 → 상품설계 → 요율산출 → 인가신청 → 출시 순으로 진행됩니다.',
        prompt: `보험상품 개발 절차와 각 단계별 주요 업무를 설명해주세요.`
      },      {
        id: 2,
        question: '위험률 산출 방법은?',
        answer: '경험통계, 생명표, 손해율 분석 등을 통해 산출합니다.',
        prompt: `보험상품 위험률 산출 방법과 고려 요소를 설명해주세요.`
      },      {
        id: 3,
        question: '사업비율 구조는?',
        answer: '신계약비, 유지비, 수금비로 구성됩니다.',
        prompt: `보험상품 사업비의 구조와 각 비용의 특성을 설명해주세요.`
      },      {
        id: 4,
        question: '예정이율 결정 요인은?',
        answer: '시장금리, 자산운용수익률, 경쟁상품 등을 고려합니다.',
        prompt: `보험상품 예정이율 결정 시 고려 요인을 설명해주세요.`
      },      {
        id: 5,
        question: '변액보험 특별계정 운용은?',
        answer: '펀드 선택, 자산배분, 리밸런싱 등을 통해 운용합니다.',
        prompt: `변액보험 특별계정의 운용 방법과 리스크 관리를 설명해주세요.`
      }
    ]
  }
];
...
export default function StudyPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('product-design-progress');
  ...
  const toggleQuestion = (questionId: string) => {
    ...
    localStorage.setItem('product-design-progress', JSON.stringify(updated));
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-cyan-50">
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <span className="text-cyan-600 font-medium">상품설계</span>
          </nav>
        </div>
      </header>
      ...
      <section className="bg-gradient-to-r from-cyan-500 to-teal-500 text-white py-8">
        <div className="max-w-4xl mx-auto px-4">
          <h1 className="text-2xl font-bold mb-2">상품설계</h1>
          <p className="text-cyan-100">보험계리사 - 상품설계 학습</p>
