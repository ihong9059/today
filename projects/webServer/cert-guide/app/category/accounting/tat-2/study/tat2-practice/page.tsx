    ...
    questions: [
      {
        id: 1,
        question: '수정신고란?',
        answer: '당초 신고한 과세표준이나 세액이 과소한 경우 추가 신고',
        prompt: `TAT 2급 시험 문제입니다.
...
상세히 설명해주세요.`
      },
      {
        id: 2,
        question: '경정청구란?',
        answer: '당초 신고한 과세표준이나 세액이 과다한 경우 환급 청구',
        prompt: `TAT 2급 시험 문제입니다.
...
상세히 설명해주세요.`
      },
      {
        id: 3,
        question: '가산세 감면 요건은?',
        answer: '법정신고기한 경과 후 1개월 내 수정신고시 90% 감면',
        prompt: `TAT 2급 시험 문제입니다.
...
상세히 설명해주세요.`
      ...
      {
        id: 4,
        question: '수정세금계산서 발급사유는?',
        answer: '착오, 환입, 계약해제, 내국신용장 사후개설 등',
        prompt: `TAT 2급 시험 문제입니다.
...
상세히 설명해주세요.`
      },
      {
        id: 5,
        question: '대손세액공제란?',
        answer: '외상매출금 등이 회수불능시 관련 부가세액을 공제받는 제도',
        prompt: `TAT 2급 시험 문제입니다.
...
export default function Tat2PracticeStudyPage() {
  const [completedQuestions, setCompletedQuestions] = useState<Record<string, boolean>>({});
  const [expandedTopics, setExpandedTopics] = useState<Record<string, boolean>>({});

  useEffect(() => {
    const saved = localStorage.getItem('tat-2-tat2-practice-progress');
  ...
  const toggleComplete = (topicId: string, questionId: number) => {
    ...
    localStorage.setItem('tat-2-tat2-practice-progress', JSON.stringify(updated));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <Link href="/category/accounting/tat-2" className="text-gray-600 hover:text-emerald-600">TAT 2급</Link>
            <span className="text-gray-300">›</span>
            <span className="text-emerald-600 font-medium">세무정보처리</span>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">세무정보처리</h1>
