...
const topics = [
  {
    ...
    questions: [
      {
        id: 1,
        question: '철근콘크리트 보의 설계 순서를 설명하시오.',
        answer: '하중산정→단면결정→철근량산정→상세설계',
        prompt: `토목기사 철근콘크리트 문제입니다.\n\n문제: 철근콘크리트 보의 설계 순서를 설명하시오.\n\n다음 순서로 설명해주세요:\n1. 핵심 개념 정리\n2. 상세 설명\n3. 적용 예시\n4. 관련 공식\n5. 연습문제 3개`
      },      {
        id: 2,
        question: '강도설계법과 허용응력설계법의 차이를 설명하시오.',
        answer: '극한강도 vs 사용하중 기준',
        prompt: `토목기사 철근콘크리트 문제입니다.\n\n문제: 강도설계법과 허용응력설계법의 차이를 설명하시오.\n\n다음 순서로 설명해주세요:\n1. 핵심 개념 정리\n2. 상세 설명\n3. 적용 예시\n4. 관련 공식\n5. 연습문제 3개`
      }
    ]
  }
];

export default function ReinforcedconcreteStudyPage() {
  const [completedQuestions, setCompletedQuestions] = useState<Record<string, boolean>>({});
  const [expandedTopics, setExpandedTopics] = useState<Record<string, boolean>>({});

  useEffect(() => {
    const saved = localStorage.getItem('reinforced-concrete-progress');
  ...
  const toggleComplete = (topicId: string, questionId: number) => {
    ...
    localStorage.setItem('reinforced-concrete-progress', JSON.stringify(newCompleted));
  };
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-300">›</span>
            <span className="text-amber-600 font-medium">철근콘크리트</span>
          </nav>
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">철근콘크리트 학습</h1>
                <p className="text-amber-100">토목기사</p>
              </div>
            </div>
