/**
 * Device Info Server - Cross Platform (Mac/Windows/Linux)
...
const SERVICES = [
  ...
  { name: 'cert-guide', port: 3000, description: '자격시험 가이드' },
  { name: 'edu-platform Frontend', port: 3002, description: 'C/ESP32 교육 플랫폼' },
  { name: 'edu-platform Backend', port: 3001, description: '교육 플랫폼 API' },
  { name: 'snu-consulting', port: 8000, description: 'SNU Consulting 웹사이트' },
  ...
  return new Promise((resolve) => {
    ...
    if (platform === 'darwin') {
      ...
      exec('sw_vers -productVersion', (err, stdout) => {
        info.osVersion = err ? os.type() + ' ' + os.release() : 'macOS ' + stdout.trim();
        resolve(info);
      });
    } else if (platform === 'win32') {
      // Windows
      exec('ver', (err, stdout) => {
        info.osVersion = err ? os.type() + ' ' + os.release() : stdout.trim();
        resolve(info);
      });
    } else {
      // Linux 등
...
<head>
  ...
  <title>Device Info - 시스템 정보</title>
  <style>
...
<body>
  <div class="container">
    <h1><span class="icon">${sysInfo.platform === 'darwin' ? '🍎' : sysInfo.platform === 'win32' ? '💻' : '🐧'}</span>${sysInfo.computerName} 시스템 정보</h1>
    ...
    <div class="card">
      ...
      <ul class="service-list">
        ${serviceStatuses.map(s => `
          <li class="service-item">
            ...
            ${s.isRunning && s.port !== 8080 && s.port !== 3001 ? `<a class="service-link" href="http://${localIP}:${s.port}" target="_blank">열기 →</a>` : ''}
          </li>
        `).join('')}
      </ul>
      ...
      <div class="guide-section">
        <h3>📋 서버 시작 방법</h3>
        <ul>
          <li><strong>SensorMonitor:</strong> <code>cd webServer/SensorMonitor && node server/server.js</code></li>
          <li><strong>cert-guide:</strong> <code>cd webServer/cert-guide && npm run dev</code></li>
          <li><strong>snu-consulting:</strong> <code>cd webServer/snu-consulting && python3 -m http.server 8000</code></li>
          <li><strong>Device Info:</strong> <code>cd webServer/device-info && node server.js</code></li>
          <li><strong>모든 서비스:</strong> <code>cd webServer && ./start-all.sh</code></li>
        </ul>
      </div>
    </div>
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🖥️  Device Info Server running on port ${PORT}`);
