...
각 슬라이드 = (이미지 × 나레이션 길이 + 0.6초 여유) + 페이드인 0.3초
최종 = 12 클립 concat
"""
import subprocess
import json
...
OUT = BASE / "동아정밀_PET두께측정기_제안.mp4"

FFMPEG = imageio_ffmpeg.get_ffmpeg_exe()
print(f"ffmpeg: {FFMPEG}")

N = 10
TAIL_PAD = 0.4  # seconds silence after each narration
FADE_IN = 0.25

def get_audio_duration(mp3_path):
    """ffmpeg로 mp3 duration 추출"""
    # ffmpeg writes the duration info to stderr
    res = subprocess.run(
...
def build_clip(idx):
    ...
    # Pad audio with silence at end (apad) so video and audio finish together
    # Video: still image looped to total_dur, with fade-in 0.3s
    # Audio: mp3 + silence pad to total_dur
    cmd = [
        FFMPEG, "-y",
