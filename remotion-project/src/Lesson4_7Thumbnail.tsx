...
export const Lesson4_7Thumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: COLORS.gradient, justifyContent: "center", alignItems: "center", fontFamily: "Pretendard, sans-serif" }}>
      ...
      <div style={{ position: "absolute", top: 40, right: 50, background: "rgba(0,0,0,0.3)", padding: "12px 30px", borderRadius: 50, fontSize: 28, color: COLORS.light, fontWeight: 600 }}>Level 4-7</div>
      <div style={{ textAlign: "center", zIndex: 10 }}>
        <div style={{ fontSize: 180, marginBottom: 20, filter: "drop-shadow(0 10px 30px rgba(0,0,0,0.3))" }}>💾</div>
        <div style={{ fontSize: 100, fontWeight: 800, color: COLORS.light, textShadow: "0 6px 30px rgba(0,0,0,0.4)", marginBottom: 20 }}>모델 저장과 로드</div>
        <div style={{ fontSize: 48, color: "rgba(255,255,255,0.9)", fontWeight: 500 }}>학습된 모델을 저장하고 다시 사용하기</div>
        <div style={{ display: "flex", gap: 20, justifyContent: "center", marginTop: 40 }}>
          {["state_dict", "체크포인트", "map_location"].map((keyword) => (
