  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: "absolute", top: 40, right: 50, padding: "15px 40px", background: `linear-gradient(135deg, ${colors.gray[700]} 0%, ${colors.gray[800]} 100%)`, borderRadius: 20, border: `3px solid ${colors.primary}50` }}>
        <span style={{ fontSize: 32, color: colors.white, fontWeight: "bold" }}>Level 1-4</span>
      </div>

      <div style={{ position: "absolute", top: "22%", left: "50%", transform: "translateX(-50%)", fontSize: 180 }}>📈</div>

      <div style={{ position: "absolute", top: "50%", left: 0, right: 0, textAlign: "center" }}>
        <span style={{ fontSize: 110, fontWeight: "bold", color: colors.white, textShadow: `0 0 30px ${colors.primary}, 0 4px 20px rgba(0,0,0,0.5)` }}>퍼셉트론 학습</span>
      </div>

      <div style={{ position: "absolute", top: "68%", left: 0, right: 0, textAlign: "center" }}>
        <span style={{ fontSize: 48, color: colors.gray[100], textShadow: "0 2px 10px rgba(0,0,0,0.5)" }}>오차로부터 배우는 가중치 업데이트</span>
      </div>

      <div style={{ position: "absolute", bottom: 80, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 25 }}>
        {["예측", "오차", "학습률", "업데이트"].map((item, i) => (
          <div key={i} style={{ padding: "15px 35px", backgroundColor: `${colors.primary}40`, borderRadius: 15, border: `3px solid ${colors.primary}` }}>
            <span style={{ fontSize: 32, color: colors.white, fontWeight: "bold" }}>{item}</span>
          </div>
        ))}
      </div>
    </AbsoluteFill>
