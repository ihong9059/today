...
export const Lesson4_4Thumbnail: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: COLORS.gradient, justifyContent: "center", alignItems: "center", fontFamily: "Pretendard, sans-serif" }}>
      ...
      <div style={{ position: "absolute", top: 40, right: 50, background: "rgba(0,0,0,0.3)", padding: "12px 30px", borderRadius: 50, fontSize: 28, color: COLORS.light, fontWeight: 600 }}>Level 4-4</div>
      <div style={{ textAlign: "center", zIndex: 10 }}>
        <div style={{ fontSize: 180, marginBottom: 20, filter: "drop-shadow(0 10px 30px rgba(0,0,0,0.3))" }}>🧱</div>
        <div style={{ fontSize: 110, fontWeight: 800, color: COLORS.light, textShadow: "0 6px 30px rgba(0,0,0,0.4)", marginBottom: 20 }}>nn.Module 기초</div>
        <div style={{ fontSize: 48, color: "rgba(255,255,255,0.9)", fontWeight: 500 }}>PyTorch 신경망의 기본 클래스</div>
        <div style={{ display: "flex", gap: 20, justifyContent: "center", marginTop: 40 }}>
          {["super()", "forward()", "parameters()"].map((keyword) => (
