  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: "absolute", top: 40, right: 50, padding: "15px 40px", background: `linear-gradient(135deg, ${colors.gray[700]} 0%, ${colors.gray[800]} 100%)`, borderRadius: 20, border: `3px solid ${colors.primary}50` }}>
        <span style={{ fontSize: 32, color: colors.white, fontWeight: "bold" }}>Level 1-6</span>
      </div>

      <div style={{ position: "absolute", top: "22%", left: "50%", transform: "translateX(-50%)", fontSize: 180 }}>❌</div>

      <div style={{ position: "absolute", top: "50%", left: 0, right: 0, textAlign: "center" }}>
        <span style={{ fontSize: 100, fontWeight: "bold", color: colors.white, textShadow: `0 0 30px ${colors.primary}, 0 4px 20px rgba(0,0,0,0.5)` }}>XOR 문제와 한계</span>
      </div>

      <div style={{ position: "absolute", top: "68%", left: 0, right: 0, textAlign: "center" }}>
        <span style={{ fontSize: 48, color: colors.gray[100], textShadow: "0 2px 10px rgba(0,0,0,0.5)" }}>퍼셉트론의 한계와 AI의 겨울</span>
      </div>

      <div style={{ position: "absolute", bottom: 80, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 25 }}>
        {[
          { type: "XOR", color: "#ef4444" },
          { type: "선형분리", color: "#3b82f6" },
          { type: "AI 겨울", color: "#6b7280" },
        ].map((item, i) => (
          <div key={i} style={{ padding: "15px 40px", backgroundColor: `${item.color}40`, borderRadius: 15, border: `3px solid ${item.color}` }}>
            <span style={{ fontSize: 36, color: colors.white, fontWeight: "bold" }}>{item.type}</span>
          </div>
        ))}
      </div>
    </AbsoluteFill>
