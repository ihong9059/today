  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: "absolute", top: 40, right: 50, padding: "15px 35px", background: "rgba(255,255,255,0.2)", borderRadius: 15, border: "2px solid rgba(255,255,255,0.4)" }}>
        <span style={{ fontSize: 32, color: colors.white, fontWeight: "bold" }}>Level 3-4</span>
      </div>
      ...
      <div style={{ position: "absolute", bottom: "22%", left: 0, right: 0, textAlign: "center" }}>
        <div style={{ fontSize: 100, fontWeight: "bold", color: colors.white, textShadow: `0 4px 30px rgba(0,0,0,0.4), 0 0 60px ${colors.primary}`, letterSpacing: 5 }}>활성화 함수</div>
        <div style={{ fontSize: 42, color: colors.gray[100], marginTop: 15, textShadow: "0 2px 10px rgba(0,0,0,0.3)" }}>Activation Functions</div>
      </div>
      <div style={{ position: "absolute", bottom: 50, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 20 }}>
        {["Sigmoid", "ReLU", "Leaky", "Softmax"].map((tag, i) => (
...
export default Lesson3_4Thumbnail;
