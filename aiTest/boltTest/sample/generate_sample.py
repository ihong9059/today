...
INPUT_PATH = os.path.join(SCRIPT_DIR, 'original.png')
GOOD_DIR = os.path.join(SCRIPT_DIR, 'good')
DEFECT_DIR = os.path.join(SCRIPT_DIR, 'defect')
...
def main():
    ...
    for i in range(3):
        ...
        path = os.path.join(GOOD_DIR, f'good_{i+1:02d}.png')
        cv2.imwrite(path, sample)
        print(f"  저장: good_{i+1:02d}.png")
    ...
    for defect_type, func in defect_funcs.items():
        ...
        cv2.imwrite(path, sample)
        print(f"  저장: defect_{defect_type}.png")

    print("\n" + "="*50)
    print("  완료!")
    print("="*50)
    print(f"  양품: {GOOD_DIR} (3개)")
    print(f"  불량: {DEFECT_DIR} (6개)")
    print(f"\n  총 9개 샘플 생성됨")
