...
> 이 예제는 앞에서 배운 **모든 자료형을 하나의 코드에 통합**하여, 실제 AI 학습이 어떻게 돌아가는지 체험해보는 코드입니다. 실행 전에 아래 표를 먼저 읽어보세요!

### 코드 속 자료형 해부

| 코드 | 자료형 | AI에서의 의미 |
|------|--------|--------------|
| \`"SimpleNet"\` | **str** (문자열) | 모델 이름 |
| \`0.01\` | **float** (실수) | 학습률 - 한 번에 얼마나 배울지 결정 |
| \`5\` | **int** (정수) | epochs - 전체 데이터를 몇 번 반복 학습할지 |
| \`[784, 128, 10]\` | **list** (리스트) | 신경망 각 층의 뉴런 수 (입력→은닉→출력) |
| \`config = {...}\` | **dict** (딕셔너리) | 하이퍼파라미터 설정을 한곳에 묶어 관리 |
| \`accuracy < 0.99\` | **bool** (참/거짓) | 조건 판단에 사용 |

### 실행하면 무슨 일이 일어나나?

1. **config 딕셔너리**로 모델 설정을 정의합니다 → 실제 AI 프로젝트에서도 이렇게 설정을 관리합니다
2. **for 반복문**으로 5번(epochs) 학습을 반복합니다 → 매번 loss(오차)는 줄고, accuracy(정확도)는 올라갑니다
3. **history 딕셔너리**에 매 epoch의 결과를 기록합니다 → 나중에 그래프로 시각화할 때 사용하는 패턴입니다

> ⚠️ **주의: 이 코드는 시뮬레이션(모의 실험)입니다!**
> \`loss *= 0.7\`과 \`accuracy += 0.1\`은 실제 데이터를 학습하는 것이 아니라, "AI 학습을 하면 이런 패턴으로 변한다"는 것을 **단순 수식으로 흉내**낸 것입니다. 출력되는 99% 정확도는 가짜 수치이며, 실제 AI에서는 진짜 데이터(예: 손글씨 이미지 6만 장)를 학습해야 의미 있는 정확도가 나옵니다. 이 예제의 목적은 정확도 자체가 아니라, **다양한 자료형이 AI 코드에서 어떻게 함께 쓰이는지** 보여주는 것입니다.

> 💡 **핵심**: 변수와 자료형을 따로따로 외우지 마세요. 이 코드처럼 **실제 AI에서는 모든 자료형이 함께 사용**됩니다!
