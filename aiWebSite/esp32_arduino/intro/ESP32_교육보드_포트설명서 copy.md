...
| IO12 | TP3 | 테스트 포인트 | - | 범용 GPIO |
| IO13 | TP4 | 테스트 포인트 | - | 범용 GPIO |
...
| IO34 | AUX | 디지털 입력 | LoRa 모듈 | 입력 전용 |
| VP (IO36) | TP1 | 테스트 포인트 | - | ADC 입력 전용 |
| VN (IO39) | TP2 | 테스트 포인트 | - | ADC 입력 전용 |
...
| 테스트포인트 | GPIO | 특징 | 권장 용도 |
|--------------|------|------|-----------|
| TP1 | VP (IO36) | ADC1_CH0, 입력 전용 | 아날로그 센서 입력 |
| TP2 | VN (IO39) | ADC1_CH3, 입력 전용 | 아날로그 센서 입력 |
| TP3 | IO14 | ADC2_CH6, PWM 가능 | 범용 입출력 |
| TP4 | IO13 | ADC2_CH4, PWM 가능 | 범용 입출력 |
...
### 4.2 부팅 시 주의 핀
- **IO0**: 부팅 모드 선택 (LOW=다운로드, HIGH=실행)
- **IO2**: 부팅 시 LOW 권장
- **IO12**: 부팅 시 HIGH면 플래시 전압 오류 가능
- **IO15**: 부팅 시 HIGH면 디버그 출력 활성화

### 4.3 내부 플래시 사용 핀 (사용 금지)
- **IO6 ~ IO11** (D0, D1, D2, D3, CMD, CLK): 내부 플래시 연결
...
```
              ...
              3V3  ─┤ 1            38 ├─ CLK (내부)
               EN  ─┤ 2            37 ├─ D0 (내부)
       TP1    VP   ─┤ 3            36 ├─ D1 (내부)
       TP2    VN   ─┤ 4            35 ├─ IO15 → M0 (LoRa)
       AUX   IO34  ─┤ 5            34 ├─ IO2
             IO35  ─┤ 6            33 ├─ IO0
    SWITCH   IO32  ─┤ 7            32 ├─ IO4 → M1 (LoRa)
    MELODY   IO33  ─┤ 8            31 ├─ IO16 → RXD2 (LoRa)
       RED   IO25  ─┤ 9            30 ├─ IO17 → TXD2 (LoRa)
    YELLOW   IO26  ─┤10            29 ├─ IO5 → SS (SPI)
      BLUE   IO27  ─┤11            28 ├─ IO18 → SCK (SPI)
      BEEP   IO14  ─┤12            27 ├─ IO19 → MISO (SPI)
       TP3   IO12  ─┤13            26 ├─ GND
              GND  ─┤14            25 ├─ IO21 → SDA (I2C)
       TP4   IO13  ─┤15            24 ├─ RX
             D2    ─┤16            23 ├─ TX
             D3    ─┤17            22 ├─ IO22 → SCL (I2C)
             CMD   ─┤18            21 ├─ IO23 → MOSI (SPI)
              5V   ─┤19            20 ├─ GND
                    └─────────────────┘
                      ESP32-DevKitC
```
...
| **테스트** | | | |
| TP1 | 36 | `TP1_PIN` | ADC 입력 |
| TP2 | 39 | `TP2_PIN` | ADC 입력 |
| TP3 | 12 | `TP3_PIN` | 범용 GPIO |
| TP4 | 13 | `TP4_PIN` | 범용 GPIO |
...
회로도 분석 결과, 다음 순서로 교육 커리큘럼을 구성하는 것을 권장합니다:

### 초급 (기초)
1. **LED 제어** - digitalWrite, 신호등 만들기
2. **PWM과 LED 밝기** - analogWrite, 페이드 효과
3. **스위치 입력** - digitalRead, 버튼으로 LED 제어
4. **부저 제어** - tone(), 비프음과 멜로디
5. **OLED 디스플레이** - I2C 통신, 텍스트/그래픽 출력
6. **온습도 센서 (AHT20)** - I2C, 센서 데이터 읽기

### 중급 (응용)
7. **아날로그 입력** - ADC, 가변저항/센서 연결 (TP1, TP2)
8. **인터럽트** - 버튼 인터럽트 처리
9. **타이머** - millis(), 논블로킹 코드
10. **SPI 통신** - SD카드 연결

### 고급 (통신)
11. **LoRa 통신 기초** - Serial2, 모드 설정
12. **LoRa 양방향 통신** - 1:1 통신
13. **Wi-Fi 연결** - HTTP 서버
14. **IoT 프로젝트** - 센서 + 디스플레이 + 통신 통합
