...
- 사용자 명확화: "uttecBizWiki는 onDevice_AI 개발 제품에 대해서만 진행. 다른 biz는 관여하지 않음."
- 정정 내용:
...
- 핵심 결론: **본 vault는 단일 제품군(onDevice AI = AI FanStick 차세대 + Stage 4 패키지) 전용**. onDevice_AI vault와 1:1 한 쌍 관계.

## [2026-05-07 17:50] start | uttecBizWiki 본격 진입 — AI FanStick 첫 사업 영역

- 결정: 5/5 작성한 검토 노트 보류 → 본격 진입
- 트리거:
  1. 5/7 onDevice_AI vault 신설 (기술 vault) → 비즈니스 vault와의 분리 명확화
...
- 핵심 결론: **3-vault 분리 운영 시작**. myWiki(학습) + uttecBizWiki(사업) + onDevice_AI(기술) 한 쌍. 같은 제품(AI FanStick / Stage 4)을 두 vault(기술↔비즈니스)에서 동시 추적.
